"""
Estrategia VWAP Breakout adaptada para Backtrader

Implementa la lógica REAL de VWAP Breakout usando el framework de backtrader
basado en el VWAPWorkerLogic: análisis VWAP, buy-the-dip system,
y validación de trend strength.
"""
...
class VWAPBreakoutStrategy(bt.Strategy):
    """
    Estrategia VWAP Breakout para backtrader usando lógica REAL del worker

    Implementa análisis VWAP completo: cálculo VWAP, validación de trend strength,
    buy-the-dip system, y filtros de momentum.
    """
    ...
    params = (
        ...
        ('enable_buy_the_dip', True), # Habilitar buy-the-dip
        ('dip_pullback_min_pct', 0.3), # Pullback mínimo (30%)
        ('dip_pullback_max_pct', 0.6), # Pullback máximo (60%)
        ('dip_timeout_minutes', 20),   # Timeout para dip
        ('dip_volume_multiplier', 1.5), # Multiplicador de volumen para dip
    )

    def __init__(self):
        """Inicializar estrategia con lógica real del worker"""
        self.logger = logging.getLogger('backtesting.vwap_breakout')

        # Inicializar el worker logic real (sin execution engine para backtesting)
        self.worker_logic = VWAPWorkerLogic(
    ...
    def next(self):
        ...
        try:
            ...
            if len(self.data) < 50:  # Necesitamos más datos para análisis VWAP completo
                return
    ...
    def _simulate_catalyst_data(self) -> Dict[str, Any]:
        ...
        Para VWAP Breakout, enfocamos en catalizadores que pueden
        crear movimientos volumétricos alrededor del VWAP
        """
        try:
            ...
            # Simular catalizadores que pueden crear VWAP breakouts
            ...
            # VWAP Breakout funciona mejor en small caps ($2-50)
    ...
    def _execute_entry(self, opportunity: Dict[str, Any]):
        ...
        try:
            ...
            self.logger.info(
                f"🎯 VWAP_BREAKOUT ENTRY: {symbol} @ ${current_price:.2f} "
    ...
    def _calculate_position_size(self, opportunity: Dict[str, Any]) -> int:
        ...
        try:
            ...
            stop_distance = price * 0.03  # 3% stop loss (VWAP usa 3%)
        ...
        try:
            for symbol, position_data in list(self.active_positions.items()):
                ...
                position = {
                    ...
                    'trading_horizon': 'INTRADAY',  # VWAP Breakout es intraday por defecto
    ...
    def _execute_exit(self, symbol: str, reason: str, current_price: float):
        ...
        try:
            ...
            self.logger.info(
                f"🔴 VWAP_BREAKOUT EXIT: {symbol} @ ${current_price:.2f} "
    ...
    def stop(self):
        ...
        self.logger.info("=" * 50)
        self.logger.info("VWAP BREAKOUT BACKTEST RESULTS (REAL WORKER LOGIC)")
        self.logger.info("=" * 50)
