...
class ExecutionEngineAdapter:
        ...
        async with self._position_locks[symbol]:
            ...
            try:
                ...
                if current_session in [MarketSession.PREMARKET, MarketSession.AFTERHOURS]:
                    ...
                    self.logger.info(f"⏰ {strategy}: Using GTC (Good Till Cancelled) TIF for extended hours execution")
                else:
                ...
                if position_metadata:
                    ...
                    # Store daily analysis metadata (if available)
    ...
    async def exit_position(
        ...
        try:
            ...
            while elapsed < max_wait_seconds and not actual_exit_price:
                ...
                # PRIORITY 2: Verify position is closed (fallback confirmation)
                if not actual_exit_price:
                    try:
                        positions = await self.broker.get_positions()
                        # Position should be gone or reduced
                        if symbol not in positions or positions[symbol].quantity < quantity:
                            # Position confirmed closed but no avgFillPrice yet, use current_price as estimate
                            actual_exit_price = current_price
                            self.logger.warning(
                                f"⚠️ {symbol} exit confirmed by position closure after {elapsed:.1f}s, "
                                f"but using estimated price ${actual_exit_price:.2f} (no avgFillPrice yet)"
                            )
                            break
                    except Exception as e:
                        self.logger.debug(f"Position check attempt {elapsed:.0f}s: {e}")

            # Fallback if order not confirmed after max wait
            if not actual_exit_price:
                actual_exit_price = current_price
                self.logger.error(
    ...
    def _format_strategy_name(self, strategy_name: str) -> str:
        ...
        strategy_map = {
            ...
            'vcp_smallcap': 'VCP-SC',
            'generic_01': 'Low Vol Accum'  # Low Volume Accumulation - 41% edge strategy
        }
            ...
            if broker_positions:
                for symbol, broker_position in broker_positions.items():
                    ...
                    self.logger.warning(
                        ...
                        f"@ ${broker_position.avg_cost:.2f} but NO trade in database"
                    )
                    ...
                    try:
                        ...
                        trade_data = {
                            ...
                            'entry_price': float(broker_position.avg_cost),
                            'entry_time': datetime.now(),  # Approximate (we don't know real entry time)
                            'status': 'OPEN',
                            'notes': f'AUTO_REGISTERED: Manual position detected in broker (avg cost ${broker_position.avg_cost:.2f})',
                            'actual_entry_price': float(broker_position.avg_cost),
                            'entry_filled': True
                        }

                        # Insert into database
                        self.db_manager.create_trade(trade_data)
                        ...
                        position_value = float(broker_position.avg_cost) * int(broker_position.quantity)
                        self._pending_unified_registrations.append({
                            'symbol': symbol,
                            'strategy_type': 'day',
                            'position_data': {
                        ...
                        self.logger.info(
                            ...
                            f"${broker_position.avg_cost:.2f} x {broker_position.quantity} shares "
    ...
    async def _wait_for_execution_confirmation(self, order_id: str, symbol: str, expected_quantity: int, timeout: float = 30.0) -> bool:
        ...
        while (asyncio.get_event_loop().time() - start_time) < timeout:
            try:
                ...
                if symbol in positions:
                    pos = positions[symbol]
                    if pos.quantity >= expected_quantity and pos.avgCost > 0:
                        self.logger.info(f"✅ Position confirmed in IBKR: {symbol} {pos.quantity} shares @ ${pos.avgCost:.2f}")
                        return True
