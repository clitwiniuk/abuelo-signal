...
class StructuralExitCalculator:
    ...
    def __init__(
        ...
        if config:
            self.min_risk_reward = config.get('min_risk_reward_ratio', 2.0)
            self.min_expected_value = config.get('min_expected_value_pct', 2.0)
            self.resistance_buffer = config.get('resistance_buffer_pct', 0.02)
            self.support_buffer = config.get('support_buffer_pct', 0.01)
            self.strong_resistance_threshold = config.get('strong_resistance_threshold', 70)
        else:
            # Default values
    ...
    def calculate_exits(
        ...
        try:
            ...
            # 1. Detectar niveles estructurales
            levels = self._detect_structural_levels(opportunity)
    ...
    def _detect_structural_levels(self, opportunity: Dict) -> Dict:
        """
        ...
        Returns:
            {
                ...
                'supports': [(price, source, strength), ...],
        ...
        # Sort supports by strength then proximity
    ...
    def _calculate_optimal_sl(
        ...
        if levels['supports']:
            ...
            # Validate reasonable distance (2-15%)
            sl_distance_pct = abs((sl_price - entry_price) / entry_price) * 100
            if 2.0 <= sl_distance_pct <= 15.0:
                return sl_price, source
