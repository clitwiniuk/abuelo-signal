...
class CentralizedStopLossManager:
        ...
        try:
            ...
            if pnl_pct >= rapid_profit_threshold:
                try:
                    ...
                    self.logger.debug(f"Timezone issue in runner detection for {position.symbol}: {e}")
    ...
    def _check_time_exits(self, position: PositionTracking, current_time: datetime) -> Optional[Dict[str, Any]]:
        ...
        try:
            # Try to make both timezone-aware if one is aware
            entry_time = position.entry_time
            curr_time = current_time

            # If one has timezone and other doesn't, normalize to naive
            ...
            self.logger.warning(f"⚠️ Timezone issue calculating hold time for {position.symbol}: {e}")
            # Fallback: assume minimal hold time to allow other exit checks
            hold_minutes = 0
