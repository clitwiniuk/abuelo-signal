...
class ContextEngine:
    ...
    def __init__(self):
        ...
        self.logger.info("🧠 ContextEngine initialized - SMALL CAP optimized thresholds")
    ...
    def detect_context(self, ticker_data: Dict) -> ContextAnalysis:
        ...
        try:
            ...
            if not bars or len(bars) < 30:
                self.logger.warning(f"⚠️ {symbol}: Insufficient bars for context ({len(bars)}/30)")
    ...
    def analyze_daily_potential(self, ticker_data: Dict) -> Dict[str, any]:
        ...
        try:
            ...
            swing_checks = {
                'rsi_ok': rsi_daily < 70,
                'resistance_space': distance_to_resistance > 15,
                'macd_positive': macd_hist > 0,
                'not_extended': consecutive_days_up < 5,
                'strong_trend': adx_daily > 22
            }
            ...
            if not swing_checks['resistance_space']:
                reasons.append(f"Too close to resistance ({distance_to_resistance:.1f}% < 15%)")
            ...
            if not swing_checks['strong_trend']:
                reasons.append(f"Weak daily trend (ADX {adx_daily:.1f} < 22)")

            # SWING_SHORT VALIDATION (1-3 days holding) - more relaxed
            swing_short_checks = {
                'rsi_ok': rsi_daily < 75,
                'resistance_space': distance_to_resistance > 8,
    ...
    def _find_nearest_resistance(self, bars: List, current_price: float) -> float:
        ...
        try:
            ...
            # Look at highs from last 30 days
            recent_highs = [bar.high for bar in bars[-30:]]
    ...
    def _find_nearest_support(self, bars: List, current_price: float) -> float:
        ...
        try:
            ...
            # Look at lows from last 30 days
            recent_lows = [bar.low for bar in bars[-30:]]
