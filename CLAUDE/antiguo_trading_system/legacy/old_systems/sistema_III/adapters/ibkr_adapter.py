...
# from core.execution_tracker import get_execution_tracker  # Temporarily disabled
...
class IBKRAdapter(IBroker, IDataProvider):
    ...
    def __init__(self, host: str = "127.0.0.1", port: int = 7497, client_id: int = 4149, config=None):
        ...
        # EXECUTION TRACKING: Temporarily disabled
    ...
    def _should_use_extended_hours(self) -> bool:
        """Determine if extended hours data should be used for calculations"""
        try:
            if self.config:
                return self.config.getboolean('MARKET_SESSIONS', 'use_extended_hours_data', fallback=False)
            return False
    ...
    def _can_execute_now(self) -> bool:
        """Check if execution is allowed during current time"""
        try:
            if self.config:
                execute_only_regular = self.config.getboolean('MARKET_SESSIONS', 'execute_only_regular_hours', fallback=True)
                if not execute_only_regular:
                    return True  # Execution allowed anytime

                # Check if we're in regular trading hours
                from datetime import datetime, time
                import pytz

                tz = pytz.timezone(self.config.get('SYSTEM', 'timezone', fallback='America/New_York'))
                now = datetime.now(tz).time()

                market_open_str = self.config.get('MARKET_SESSIONS', 'market_open', fallback='09:30')
                market_close_str = self.config.get('MARKET_SESSIONS', 'market_close', fallback='16:00')

                market_open = time(*map(int, market_open_str.split(':')))
                market_close = time(*map(int, market_close_str.split(':')))

                return market_open <= now <= market_close

            return True  # Default to allowing execution
        except Exception as e:
            self.logger.warning(f"Could not determine execution hours: {e}")
            return True  # Default to allowing execution
    ...
    def is_connected(self) -> bool:
        """Check if connected to IBKR.
        Avoid calling ib.isConnected() from another thread, rely on internal flag.
        """
    ...
    async def place_order(self, order: Order) -> str:
        ...
        try:
            ...
            if not self._can_execute_now():
                raise ValueError("Order execution not allowed outside regular trading hours")
    ...
    async def place_bracket_order(self, symbol: str, action: str, quantity: int,
                                 ...
                                 take_profit_price: Optional[float] = None) -> Dict[str, str]:
        """
        Place a bracket order (parent + stop loss + take profit)

        Returns:
            Dict with order IDs: {'parent': 'xxx', 'stop': 'yyy', 'target': 'zzz'}
        """
        try:
            if not self.is_connected():
                raise BrokerConnectionError("Not connected to IBKR")
            ...
            if take_profit_price:
                target_order = IBOrder()
                target_order.orderId = target_id
                target_order.action = "SELL" if action == "BUY" else "BUY"
            ...
            # Place orders (parent first)
            result_orders = {}

            # Place parent order
            parent_trade = self.ib.placeOrder(contract, parent_order)
            result_orders['parent'] = str(parent_id)
            self.logger.info(f"✅ Parent order placed: {action} {quantity} {symbol} (ID: {parent_id})")

            # Place stop loss order
            if stop_order:
                stop_trade = self.ib.placeOrder(contract, stop_order)
                result_orders['stop'] = str(stop_id)
                self.logger.info(f"🛡️ Stop loss order placed: ${stop_loss_price} (ID: {stop_id})")

            # Place take profit order
            if target_order:
                target_trade = self.ib.placeOrder(contract, target_order)
                result_orders['target'] = str(target_id)
                self.logger.info(f"🎯 Take profit order placed: ${take_profit_price} (ID: {target_id})")

            # Transmit all orders (set the last one to transmit=True)
            if target_order:
                target_order.transmit = True
                self.ib.placeOrder(contract, target_order)
            elif stop_order:
            ...
            self.logger.info(f"📊 Bracket order completed for {symbol} - Parent: {parent_id}")
    ...
    async def get_bars(self, symbol: str, timeframe: str, count: int) -> List[MarketData]:
        ...
        try:
            ...
            self.logger.debug(f"🔍 Starting data request for {symbol} ({timeframe}, {count} bars)")
    ...
    def _calculate_duration(self, count: int, timeframe: str) -> str:
        ...
        try:
            ...
            # For hourly data, request 1 week
                ...
                return "1 W"
            # For daily data, request 1 month
    ...
    def _on_execution(self, trade, fill):
        ...
        try:
            ...
            # ENHANCED: Track real execution prices for slippage analysis (disabled)
