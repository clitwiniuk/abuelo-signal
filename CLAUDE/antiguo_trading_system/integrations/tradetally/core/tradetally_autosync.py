...
class TradeTallyAutoSync:
    ...
    def connect(self):
        ...
        try:
            ...
            self.sio.connect(ws_url, socketio_path='/ws/sync/socket.io', wait_timeout=10)
    ...
    def _get_trades_from_db(self, since: Optional[datetime] = None):
        ...
        try:
            ...
            if since:
                query = "SELECT * FROM trades WHERE updated_at > ? ORDER BY updated_at DESC"
                cursor.execute(query, (since.isoformat(),))
            else:
                query = "SELECT * FROM trades ORDER BY updated_at DESC"
    ...
    def sync_trade(self, trade_id: str, notes: Optional[str] = None, tags: Optional[list] = None):
        """
        Sync single trade
