...
class IBKRNativeScanner:
    ...
    def _get_scanner_configurations(self) -> List[Dict[str, Any]]:
        ...
        """
        return [
            # ❌ DISABLED: GAP_UP scanner not available in IBKR account (Error 162)
            # {
            #     'name': 'gap_up_movers',
            #     'scan_code': 'GAP_UP',
            #     'instrument': 'STK',
            ...
            #     'exclude_convertible': True
            # },
            # ❌ DISABLED: GAP_DOWN scanner not available in IBKR account (Error 162)
            # {
            ...
            # ❌ TEMPORARILY DISABLED: rel_vol_intraday always times out (Error 162)
    ...
    async def _enhance_scan_results(self, results: List[IBKRScanResult]) -> List[IBKRScanResult]:
        ...
        for i, result in enumerate(results):
            try:
                ...
                # Get 1-minute bars for pattern detection by workers
