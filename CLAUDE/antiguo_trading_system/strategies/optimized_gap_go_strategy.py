...
class OptimizedGapGoStrategy(BaseStrategy):
    ...
    def __init__(self, parameters: Dict[str, Any] = None):
        # All parameters ML-configurable
        default_params = {
            # Gap Detection (ML-configurable)
            'gap_percent_threshold': 0.8,
            'min_gap_percent': 1.2,
            'max_gap_percent': 30.0,
            'gap_confirmation_bars': 0,
            
            # Volume Requirements (ML-configurable)
            'volume_multiplier': 1.3,
            'min_volume': 8000,
            'volume_confirmation_bars': 1,
            'min_daily_volume': 30000,
            
            # Entry Conditions (ML-configurable)
            ...
            # Timing Controls (ML-configurable)
            'market_open_hour': 9.5,
            'no_entry_after': 13.0,
            'max_hold_time': 90,
            'cooldown_period': 60,
            
            # Trading Limits (ML-configurable)
            'max_daily_trades': 5,
            ...
            # Commission & Costs (ML-configurable)
            'commission_per_share': 0.005,
            'min_commission': 1.0,
            'slippage_bps': 5,
            
            # Scanner Integration (ML-configurable)
        ...
        # Enable FOMO detection for OptimizedGapGo (ML-configurable)
        fomo_config = {
        ...
        if self.enable_fomo_exit(fomo_config):
            self.logger.info("🎪 FOMO Detection enabled for Optimized Gap & Go Strategy")
        else:
    ...
    async def add_scanner_gap(self, symbol: str, gap_data: Dict) -> None:
        ...
        self.scanner_gaps[symbol] = {
            ...
            'timestamp': gap_data.get('timestamp', datetime.now(ZoneInfo("America/New_York"))),
    ...
    async def _check_exit_conditions(self, symbol: str, bar: MarketData) -> Optional[Signal]:
        ...
        # 5. MARKET CLOSE - convert to US Eastern timezone
    ...
    def _calculate_position_size(self, price: float) -> int:
        ...
        try:
            max_value = float(self._parameters.get('max_position_value', 400.0))
            min_value = float(self._parameters.get('min_position_value', 80.0))
            min_qty = int(self._parameters.get('min_quantity', 5))
            
            if price <= 0:
                return min_qty
            
            # Target value (punto medio)
            ...
            self.logger.debug(f"Position sizing: price=${price:.2f}, qty={quantity}, "
                             f"value=${position_value:.2f} (target=${target_value:.2f})")
            
            return max(quantity, 1)
            
        except Exception as e:
            self.logger.error(f"Error calculating position size: {e}")
            return int(self._parameters.get('min_quantity', 5))
    ...
    async def on_position_update(self, position: Position) -> Optional[Signal]:
        ...
        # For OptimizedGapGo strategy, we don't need special logic on position updates
        return None
