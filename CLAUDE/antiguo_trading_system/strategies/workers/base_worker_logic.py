...
class BaseWorkerLogic(ABC):
    ...
    def __init__(
        ...
        self.logger.info(f"🔧 Worker {worker_name} initialized with separate logging + ODS support + Event Logger")
        ...
        try:
            ...
            if 'bars_daily' not in opportunity or not opportunity['bars_daily']:
                ...
                bars_daily = await self.execution_engine.broker.ib.reqHistoricalDataAsync(
                    contract,
                    endDateTime='',
                    durationStr='60 D',  # 60 days of daily bars
            ...
            if bars_daily and len(bars_daily) >= 30:
                ...
                self.logger.info(
                    ...
                    f"resistance={daily_potential['distance_to_resistance']:.1f}%"
                )
            else:
                self.logger.warning(
    ...
    def is_within_entry_hours(self, symbol: str = "") -> Tuple[bool, float]:
        """
        ...
        try:
            ...
            market_open = 9.5  # Fallback to 09:30
        ...
        if not is_valid and symbol:
            ...
            self.logger.warning(
                f"⏰ Entry rejected{symbol_str}: Current time {current_hour:.2f} "
                f"outside allowed window [{market_open:.2f} - {no_entry_after:.2f}]"
            )
    ...
    async def _execute_entry(self, opportunity: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            try:
                ...
                if targets['risk_reward'] < 1.0:
                    self.logger.warning(
                        f"❌ {symbol}: REJECTED - Poor risk/reward ratio ({targets['risk_reward']:.2f}) - "
                        f"TP={targets['take_profit_pct']:.1f}% < SL={targets['stop_loss_pct']:.1f}%"
                    )
                    return False
    ...
    def validate_vwap_strength(
        ...
        try:
            ...
            if quality_score >= 85:  # A+ setups
                vwap_tolerance_pct = max(vwap_tolerance_pct, 3.5)  # At least 3.5%
            elif quality_score >= 75:  # A setups
                vwap_tolerance_pct = max(vwap_tolerance_pct, 2.5)  # At least 2.5%
