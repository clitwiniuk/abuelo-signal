...
try:
    ...
    try:
        ...
        raise ImportError("Could not import BaseStrategy. Make sure it exists in 'strategies/base.py' or 'strategies/base_strategy.py'")
...
def _auto_register_strategies():
    ...
    # MACDV Smallcaps
    try:
        from .macdv_smallcaps_strategy import MACDVSmallcapsStrategy
        register_strategy('macdv_smallcaps', MACDVSmallcapsStrategy)
        logger.info("✅ Auto-registered MACDV Smallcaps strategy")
    except ImportError as e:
        logger.debug(f"Could not auto-register MACDV Smallcaps strategy: {e}")
    ...
    # Ascending Triangle
    try:
        from .ascending_triangle_strategy import AscendingTriangleStrategy
        register_strategy('ascending_triangle', AscendingTriangleStrategy)
        logger.info("✅ Auto-registered Ascending Triangle strategy")
    except ImportError as e:
        logger.warning(f"Could not auto-register Ascending Triangle strategy: {e}")
...
# Register SimpleStrategyEngine manually (NO ML)
try:
    from .simple_strategy_engine import SimpleStrategyEngine
    register_strategy('simple_strategy_engine', SimpleStrategyEngine)
    logger.info("✅ SimpleStrategyEngine registered (NO ML dependencies)")
except ImportError as e:
    logger.warning(f"⚠️ Could not register SimpleStrategyEngine: {e}")
