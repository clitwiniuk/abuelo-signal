...
class IndependentScanner:
    ...
    def __init__(self):
        ...
        self.logger.info("✅ Pattern classifiers initialized (ODS + Intraday Structure)")
    ...
    async def start(self):
        ...
        while self.is_running and not self.shutdown_requested:
            try:
                ...
                optimized_scan_interval = max(15, base_scan_interval // 2)  # Minimum 15s, half of original
                scan_interval = optimized_scan_interval

                self.logger.info(f"⏱️ OPTIMIZED: Waiting {scan_interval}s until next scan (was {base_scan_interval}s)")
    ...
    async def _stream_opportunity_if_new(self, opportunity: Dict[str, Any]):
        ...
        # NEW OPPORTUNITY or significant price change - send immediately
        try:
            await self.bridge.publish_opportunities([opportunity])
            self.sent_opportunities[symbol] = (current_time, current_price)
            self.logger.info(f"⚡ STREAM: Sent {symbol} @ ${current_price:.2f} immediately (Q={opportunity['quality_score']:.1f})")
        except Exception as e:
        ...
        try:
            ...
            if intraday_plays:
                ...
                for play in intraday_plays:
                    ...
                    # ================================================================
                    # ENHANCEMENT 4: Enhanced Quality Score
                    # ================================================================
