...
class EODOHLCDownloader:
    ...
    def __init__(self, db_path: str = None, update_all: bool = False, target_date: str = None, force: bool = False):
        ...
        self.force = force # Force download even if fresh
    ...
    async def initialize(self):
        ...
        try:
            ...
            # Trader usa 6000, este script usa 8001 para evitar conflictos
            self.ibkr = IBKRAdapter(
                host="127.0.0.1",
                port=7497,  # Paper trading
                client_id=8001  # ID único para EOD downloads (evita conflicto con trader:6000)
            )
    ...
    def _is_data_fresh(self, trade_id: str) -> bool:
        ...
        try:
            ...
            # Si se actualizó hace menos de 4 horas, consideramos que está fresco
            # Esto evita re-descargar constantemente si el script corre múltiples veces error del usuario o reinicios
            return (datetime.now() - updated_at).total_seconds() < 14400 # 4 horas
    ...
    async def download_symbol_ohlc(self, symbol: str, trade_id: str) -> bool:
        ...
        Returns:
            bool: True si exitoso (o si ya estaba fresco)
        """
        try:
            # FRESHNESS CHECK: Evitar descargas redundantes (UNLESS FORCED)
            if not self.force and self._is_data_fresh(trade_id):
                self.logger.info(f"⏭️  Data for {symbol} ({trade_id[:8]}...) is fresh (updated < 4h). Skipping download.")
                return True

            self.logger.info(f"📥 Downloading extended OHLC data for {symbol} (Phase: {self.phase})...")

            # Calculate end_date based on target_date if available
            end_date = None
            if self.target_date:
                # Set end_date to end of the target date to capture full day + extended hours
                try:
                    target_dt = datetime.strptime(self.target_date, "%Y-%m-%d")
                    # Set to end of day 23:59:59
                    end_date = target_dt.replace(hour=23, minute=59, second=59)
                except ValueError:
                    self.logger.warning(f"⚠️ Invalid target_date format: {self.target_date}. Using current time.")
            ...
            bars = await self.ibkr.get_bars(
                ...
                end_date=end_date  # Specify end date for historical fetches
            )
            ...
            self.logger.info(f"✅ Downloaded {len(bars)} bars for {symbol} (extended hours) ending {end_date or 'NOW'}")
