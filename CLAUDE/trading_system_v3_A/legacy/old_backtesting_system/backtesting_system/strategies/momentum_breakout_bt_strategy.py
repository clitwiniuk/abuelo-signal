"""
Estrategia Momentum Breakout adaptada para Backtrader

Implementa la lógica REAL de Momentum Breakout usando el framework de backtrader
basado en el MomentumBreakoutWorkerLogic: análisis profesional de breakouts,
estructura de precio, volumen y calidad del setup.
"""
...
try:
    ...
    class MomentumBreakoutWorkerLogic:
        ...
        def should_enter(self, opportunity):
            ...
            return volume_ratio >= 1.2 and catalyst_strength >= 4
...
class MomentumBreakoutStrategy(bt.Strategy):
    """
    Estrategia Momentum Breakout para backtrader usando lógica REAL del worker

    Implementa análisis profesional de breakouts: estructura de precio,
    calidad del breakout, confirmación de volumen, y validación de uptrend.
    """
    ...
    params = (
        ...
        ('lookback_period', 20),      # Período de lookback para breakouts
        ('breakout_threshold', 2.0),  # Threshold para breakout
        ('volume_multiplier', 1.5),   # Multiplicador de volumen
    )

    def __init__(self):
        """Inicializar estrategia con lógica real del worker"""
        self.logger = logging.getLogger('backtesting.momentum_breakout')

        # Inicializar el worker logic real (sin execution engine para backtesting)
        self.worker_logic = MomentumBreakoutWorkerLogic(
    ...
    def next(self):
        ...
        try:
            ...
            if len(self.data) < 50:  # Necesitamos más datos para análisis profesional
                return
    ...
    def _simulate_catalyst_data(self) -> Dict[str, Any]:
        ...
        Para Momentum Breakout, enfocamos en catalizadores que pueden
        crear breakouts (no necesariamente técnicos puros)
        """
        try:
            ...
            # Simular catalizadores que pueden crear momentum breakouts
            ...
            # Momentum Breakout funciona mejor en small caps ($1-25)
            if 1.0 <= current_price <= 25.0:
                quality_score += 10
    ...
    def _execute_entry(self, opportunity: Dict[str, Any]):
        ...
        try:
            ...
            self.logger.info(
                f"🎯 MOMENTUM_BREAKOUT ENTRY: {symbol} @ ${current_price:.2f} "
    ...
    def _calculate_position_size(self, opportunity: Dict[str, Any]) -> int:
        ...
        try:
            ...
            stop_distance = price * 0.04  # 4% stop loss (Momentum Breakout usa 4%)
        ...
        try:
            for symbol, position_data in list(self.active_positions.items()):
                ...
                position = {
                    ...
                    'trading_horizon': 'INTRADAY',  # Momentum Breakout es intraday por defecto
    ...
    def _execute_exit(self, symbol: str, reason: str, current_price: float):
        ...
        try:
            ...
            self.logger.info(
                f"🔴 MOMENTUM_BREAKOUT EXIT: {symbol} @ ${current_price:.2f} "
    ...
    def stop(self):
        ...
        self.logger.info("=" * 50)
        self.logger.info("MOMENTUM BREAKOUT BACKTEST RESULTS (REAL WORKER LOGIC)")
        self.logger.info("=" * 50)
