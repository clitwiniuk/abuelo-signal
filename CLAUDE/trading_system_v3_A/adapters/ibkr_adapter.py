...
class IBKRAdapter(IBroker, IDataProvider):
    ...
    async def get_short_data(self, symbol: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Request market data with generic tick 236 (Inventory/Shares available to short)
            # Note: Tick 429 is no longer valid for STK contracts
            ...
            for _ in range(20):
                if getattr(ticker, 'shortableShares', None) is not None or hasattr(ticker, 'shortable'):
                    break
                await asyncio.sleep(0.1)

            # Extract data safely using getattr
            import math
            shortable_shares_raw = getattr(ticker, 'shortableShares', 0)
            shortable_val = getattr(ticker, 'shortable', 0.0)

            # Handle NaN values
            if shortable_shares_raw is None or (isinstance(shortable_shares_raw, float) and math.isnan(shortable_shares_raw)):
                shortable_shares = 0
            else:
                shortable_shares = shortable_shares_raw or 0

            if shortable_val is None or (isinstance(shortable_val, float) and math.isnan(shortable_val)):
                shortable_val = 0.0
