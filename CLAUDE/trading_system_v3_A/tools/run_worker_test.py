...
WORKER_MAP = {
    ...
    'smallcap_vwap_runner': SmallcapVwapRunnerWorker
}
...
class JsonLogHandler(logging.Handler):
    ...
    def emit(self, record):
        try:
            ...
            # Prepare log entry for frontend
            # Use ISO format to simplify frontend parsing
            if self.current_bar_time:
                ts = self.current_bar_time.isoformat()
            else:
...
def get_market_bars(symbol, date_str):
    ...
    # --- NEW FALLBACK: Check v3_A (Production) Database ---
    if not rows and not (row and row[0]):
         prod_db_path = PROJECT_ROOT.parent / 'trading_system_v3_A' / 'trading_data.db'
         if prod_db_path.exists():
             try:
                 conn_prod = sqlite3.connect(str(prod_db_path))
                 cursor_prod = conn_prod.cursor()
                 
                 # Try Snapshots in Prod
                 cursor_prod.execute(query, (symbol, date_str))
                 row_prod = cursor_prod.fetchone()
                 
                 if row_prod and row_prod[0]:
                     import json as json_lib
                     bars_data = json_lib.loads(row_prod[0])
                     if bars_data:
                         from strategies.workers.base_worker_logic import BarDataWrapper
                         bars_objects = []
                         for b in bars_data:
                 ...
                 # Try Intraday Bars in Prod
                 cursor_prod.execute(query_fallback, (symbol, date_str))
                 rows_prod = cursor_prod.fetchall()
                 conn_prod.close()
                 
                 if rows_prod:
                     rows = rows_prod # Use prod rows
             except Exception as e:
                 print(f"Warning: Failed to fetch from Prod DB: {e}", file=sys.stderr)
...
def get_scanner_signals(symbol, date_str):
    """
    Get all scanner signals for a symbol on a specific date to simulate realistic scanner behavior.
    
    Returns:
        List of dicts: [{'timestamp': datetime, 'quality_score': float, 'gap': float, 'vol': float}]
    """
    db_path = PROJECT_ROOT / 'trading_data.db'
    # Fallback to prod db if needed
    if not db_path.exists():
         prod_db = PROJECT_ROOT.parent / 'trading_system_v3_A' / 'trading_data.db'
         if prod_db.exists():
             db_path = prod_db
         else:
             return []
    ...
    try:
        ...
        query = """
            SELECT timestamp, quality_score, gap_percentage, volume_ratio
        ...
        for r in rows:
            ...
            try:
                ...
                signals.append({
                    'timestamp': dt,
                    'quality_score': r[1] or 0,
                    'gap_percentage': r[2] or 0.0,
        ...
        print(f"Warning: Could not fetch scanner signals: {e}", file=sys.stderr)
        signals = []

    # Merge Real Trades to ensure we simulate what actually happened
    # helpful if scanner logs are missing for manual/pre-market trades
    try:
        ...
        for t in real_trades:
            ...
            try:
                 ...
                 # Check if this trade is covered by an existing signal (within 30 mins prior)
                 # using naive comparison if tz-aware vs naive issues arise, normalize to UTC
                 if entry_dt.tzinfo is None: entry_dt = entry_dt.replace(tzinfo=timezone.utc)
                 
                 covered = False
                 for s in signals:
                     sig_dt = s['timestamp']
                     if sig_dt.tzinfo is None: sig_dt = sig_dt.replace(tzinfo=timezone.utc)
                     
                     diff = (entry_dt - sig_dt).total_seconds()
                     if 0 <= diff <= 3600: # Signal up to 60 mins before
                         covered = True
                         break
                 
                 if not covered:
                     # Add synthetic signal
                     synth_dt = entry_dt - timedelta(minutes=2)
                     signals.append({
                         'timestamp': synth_dt,
                         'quality_score': 85.0, # High score to ensure entry
                         'gap_percentage': 5.0,
                         'volume_ratio': 2.0,
                         'source': 'REAL_TRADE_SYNTHETIC'
                     })
            except Exception as ex:
                # heuristics might fail on weird formats
                pass
    except Exception as e:
        print(f"Warning merging real trades: {e}", file=sys.stderr)
...
async def run_worker_simulation(worker_name, symbol, date_str, momentum_indicator=None, extended_hours=False, config_overrides=None, cached_data=None):
    """
    ...
    class MockConfig:
        def __init__(self):
            ...
            # Apply extended hours overrides
            self.enable_extended_hours_trading = extended_hours
    ...
    try:
        ...
        # Pre-fetch scanner signals for realistic simulation
        scanner_signals = get_scanner_signals(symbol, date_str)
        ...
        if scanner_detection_time:
            # Check count
            count_signals = len(scanner_signals)
            worker.logger.info(f"📡 Scanner first detected {symbol} at {scanner_detection_time} (Found {count_signals} signals)")
        ...
        for i in range(len(bars)):
            ...
            if i >= WARMUP_PERIOD:
                # Dynamic Scanner Attributes based on Time
                # Use the latest signal available at or before current_bar.timestamp
                
                # Default values (if no scanner data found at all, revert to test defaults for debugging)
                current_signal = None
                
                if scanner_signals:
                    # Filter relevant signals
                    # COMPATIBILITY: Ensure timezone naiveness for comparison to avoid crashes
                    # We assume both are UTC-based or consistent enough for "latest signal" logic
                    def to_naive(dt):
                        return dt.replace(tzinfo=None) if dt.tzinfo else dt
                        
                    current_ts_naive = to_naive(current_bar.timestamp)
                    valid_signals = [s for s in scanner_signals if to_naive(s['timestamp']) <= current_ts_naive]
                    if valid_signals:
                        current_signal = valid_signals[-1]
                
                if current_signal:
                    q_score = current_signal['quality_score']
                    gap = current_signal['gap_percentage']
                    vol_r = current_signal['volume_ratio']
                elif not scanner_signals:
                    # No data found in DB -> Manual Test Mode -> Use generous defaults
                    q_score = 80
                    gap = 5.0
                    vol_r = 2.0
                else:
                    # Data exists but hasn't arrived yet -> Q=0 (No entry)
                    q_score = 0
                    gap = 0.0
                    vol_r = 0.0
                ...
                opportunity = {
                    ...
                    'gap_percentage': gap, 
                    'volume_ratio': vol_r,
                    'quality_score': q_score, # Dynamic Quality
...
if __name__ == "__main__":
    ...
    # New args for sizing
    parser.add_argument('--pos-value', type=float, help='Fixed position value')
    parser.add_argument('--pos-size', type=int, help='Fixed position size')
    ...
    # Suppress STDOUT for the simulation to avoid log pollution
    # only allow the final JSON print
    final_output = ""
    ...
    try:
        ...
        # Serialize to string
        final_output = json.dumps(result, cls=DateTimeEncoder)
    ...
    # Print ONLY the JSON to original stdout (clean)
    print(final_output, file=original_stdout)
