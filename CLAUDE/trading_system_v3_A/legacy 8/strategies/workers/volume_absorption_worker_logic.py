...
class VolumeAbsorptionWorkerLogic(BaseWorkerLogic):
    ...
    def __init__(self, worker_name, execution_engine, risk_manager=None, config=None):
        super().__init__(
        ...
        if config and hasattr(config, 'getfloat'):
            ...
            self.min_price = config.getfloat(section, 'min_price', fallback=1.0)  # RELAXED: 2.0 -> 1.0
            self.max_price = config.getfloat(section, 'max_price', fallback=25.0) # RELAXED: 20.0 -> 25.0
            self.min_avg_volume = config.getint(section, 'min_avg_volume', fallback=20000) # OPTIMIZED: 100k -> 20k (Catch sleepers)
            self.trading_start_hour = config.getfloat(section, 'trading_start_hour', fallback=9.5)
            self.trading_end_hour = config.getfloat(section, 'trading_end_hour', fallback=16.0)
            ...
            self.consolidation_range_pct = config.getfloat(section, 'consolidation_range_pct', fallback=0.03) # RELAXED: 2% -> 3%
            ...
            self.absorption_volume_mult = config.getfloat(section, 'absorption_volume_mult', fallback=1.5) # RELAXED: 2.0 -> 1.5
            ...
            self.breakout_volume_mult = config.getfloat(section, 'breakout_volume_mult', fallback=2.0) # RELAXED: 3.0 -> 2.0
            ...
            self.surveillance_volume_threshold = config.getfloat(section, 'surveillance_volume_threshold', fallback=2.5)  # OPTIMIZED: 1.5x -> 2.5x
            ...
            self.surveillance_volume_threshold = 2.5  # OPTIMIZED: 2.5x
        ...
        # AGOTAMIENTO / EXHAUSTION FILTERS (Nuevo Módulo)
        ...
        self.order_flow = None # Se inicializa lazy o si tenemos execution_engine disponible
    ...
    async def should_enter(self, opportunity: Dict) -> bool:
        ...
        4. ODS Filters (Standardized)
        5. Time Checks (Standardized)
        """
        symbol = opportunity.get('symbol', 'UNKNOWN')

        try:
            # 1. ODS Filter (Standardized)
            # Pass empty bars initially? No, we need bars.
            # get_bars_from_opportunity is usually called inside.
            # Let's get bars first.
            ...
            # Uses BaseWorkerLogic.is_within_entry_hours which supports replay/backtesting
            ...
            # 3. Basic Filters (Price, Gap, etc - usually handled by scanner, but good to double check)
            # ... existing logic continues ...

            # NOTE: I need to be careful not to delete the rest of the method.
            # The original code had:
            # symbol = opportunity['symbol']
            # if not self._is_trading_hours(): ...
            
            # I am replacing up to line 216 roughly.
            ...
            is_valid_hours, current_time_et = self.is_within_entry_hours(symbol, opportunity.get('timestamp'))
            if not is_valid_hours:
                self.logger.info(f"⚪ {symbol}: Outside trading hours (current ET: {current_time_et:.2f}h)")
                return False
            ...
            # STEP 5.6: ID EXHAUSTION - Evitar buying climax (mechas largas, extension parabolica)
            ...
            # STEP 8: Filtro de confianza mínima - Rechazar setups de baja calidad
            # Análisis 2025-12-04: trades con <50% confianza perdieron -19.24%
            # Este filtro previene entradas de baja probabilidad
            ...
            # Verificar si el volumen es compradores agresivos (Ask) vs vendedores ocultos (Bid)
            
            # Solo verificar en live/paper (requiere broker conexión)
            if self.execution_engine and getattr(self.execution_engine, 'broker', None):
                # Lazy init del analyzer si no existe
                if not self.order_flow:
                    from .orderflow_metrics import OrderFlowAnalyzer
                    self.order_flow = OrderFlowAnalyzer(self.execution_engine.broker, self.logger)
