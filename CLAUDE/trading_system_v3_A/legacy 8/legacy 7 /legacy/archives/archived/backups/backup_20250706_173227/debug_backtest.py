...
def select_strategy() -> tuple:
    ...
    while True:
        choice = input("\n👉 Selecciona la estrategia a debuggear (1-5): ").strip()
...
async def debug_backtest():
    ...
    class EnhancedDiagnosticStrategy(strategy_class):
        ...
        async def on_bar(self, bar: MarketData):
            """Método on_bar CORREGIDO - VERSION FINAL"""
            symbol = bar.symbol
            ...
            try:
                ...
                if signal:
                    ...
                    if signal_key in self.last_signals:
                        ...
                        return None  # Ignore duplicates completely
                    ...
                    # CORRECT COUNTING - Use separate counters
                    self.signal_count = int(self.signal_count) + 1
                    
                    # Identify signal type by metadata
                    is_entry = signal.metadata.get('is_entry', False) if signal.metadata else False
                    is_exit = signal.metadata.get('is_exit', False) if signal.metadata else False
                    
                    # Determine if it's entry or exit based on signal type and metadata
                    if is_exit or 'EXIT' in signal_type_str:
                        self.exit_signal_count = int(self.exit_signal_count) + 1
                        print(f"\n🔴 EXIT #{self.exit_signal_count}: {symbol} @ ${bar.close:.2f}")
                        print(f"   Razón: {signal.metadata.get('reason', 'unknown') if signal.metadata else 'unknown'}")
                        print(f"   P&L: {signal.metadata.get('pnl_pct', 0)*100:.1f}% (${signal.metadata.get('pnl_dollars', 0):.2f})" if signal.metadata else "   P&L: N/A")
                        print(f"   Size: {signal.metadata.get('position_size', 100)} shares" if signal.metadata else "   Size: 100 shares")
                        print(f"   Time: {signal.metadata.get('time_held_minutes', 0):.1f}min" if signal.metadata else "   Time: 0.0min")
                        
                    elif is_entry or signal_type_str in ['LONG', 'SHORT']:
                        self.entry_signal_count = int(self.entry_signal_count) + 1
                        print(f"\n🟢 ENTRY #{self.entry_signal_count}: {symbol} @ ${bar.close:.2f}")
                        print(f"   Gap: {signal.metadata.get('gap_percent', 0):.1f}%" if signal.metadata else "   Gap: N/A")
                        print(f"   Stop: ${signal.metadata.get('stop_loss', 0):.2f}" if signal.metadata else "   Stop: N/A")
                        print(f"   Target: ${signal.metadata.get('take_profit', 0):.2f}" if signal.metadata else "   Target: N/A")
                    
                    # Note: Do NOT increment self.entry_signals (that's a dict from strategy)
                    # We use self.entry_signal_count instead
    ...
    for i, symbol in enumerate(available_symbols, 1):
        try:
            ...
            # Considerar el símbolo como procesado si tenemos resultados, incluso sin trades
            if symbol_results:
                ...
                if hasattr(symbol_results, 'trades') and symbol_results.trades:
                    if all_results is None:
                        ...
                        # Combinar resultados (implementar lógica de combinación según tu estructura)
                        all_results.trades.extend(symbol_results.trades)
                        all_results.total_trades += len(symbol_results.trades)
                        # Actualizar otras métricas según sea necesario
                elif all_results is None:
                    # Si no hay trades pero es el primer símbolo con resultados, inicializar all_results
                    all_results = symbol_results
