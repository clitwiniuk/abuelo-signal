...
class BuyTheDipWorkerLogic(BaseWorkerLogic):
    ...
    def __init__(self, worker_name, execution_engine, risk_manager, config=None):
        ...
        if config:
            ...
            self.min_quality_score = config.getfloat(section, 'min_quality_score', fallback=40.0)

            # ===== CONFIRMACIÓN DE TENDENCIA ALCISTA =====
            self.min_price_above_vwap_pct = config.getfloat(section, 'min_price_above_vwap_pct', fallback=1.0)
            self.min_vwap_slope = config.getfloat(section, 'min_vwap_slope', fallback=0.0001)
            self.max_rsi_entry = config.getfloat(section, 'max_rsi_entry', fallback=75.0)

            # ===== DUAL MODE: BUY THE DIP vs BUY AND HOLD =====
            self.strong_trend_threshold_pct = config.getfloat(section, 'strong_trend_threshold_pct', fallback=2.0)

            # ===== PREDICCIÓN DE DIP (volatilidad-based) =====
            # Low volatility (ATR < threshold)
            self.low_vol_atr_threshold = config.getfloat(section, 'low_vol_atr_threshold', fallback=3.0)
            self.low_vol_dip_min = config.getfloat(section, 'low_vol_dip_min', fallback=1.0)
            self.low_vol_dip_max = config.getfloat(section, 'low_vol_dip_max', fallback=3.0)

            # Medium volatility
            self.med_vol_atr_threshold = config.getfloat(section, 'med_vol_atr_threshold', fallback=6.0)
            self.med_vol_dip_min = config.getfloat(section, 'med_vol_dip_min', fallback=3.0)
            self.med_vol_dip_max = config.getfloat(section, 'med_vol_dip_max', fallback=5.0)

            # High volatility
            self.high_vol_dip_min = config.getfloat(section, 'high_vol_dip_min', fallback=5.0)
            self.high_vol_dip_max = config.getfloat(section, 'high_vol_dip_max', fallback=8.0)

            # ===== CONFIRMACIÓN DE REBOTE =====
            self.min_bounce_bars = config.getint(section, 'min_bounce_bars', fallback=2)
            self.min_bounce_volume_ratio = config.getfloat(section, 'min_bounce_volume_ratio', fallback=1.2)

            # ===== LÍMITES DE ESPERA =====
            self.max_wait_minutes = config.getint(section, 'max_wait_minutes', fallback=60)
            self.max_entry_attempts = config.getint(section, 'max_entry_attempts', fallback=2)

            # ===== DYNAMIC TAKE PROFIT (Hybrid: Resistance + ATR) =====
            self.low_vol_tp_pct = config.getfloat(section, 'low_vol_tp_pct', fallback=8.0)
            self.med_vol_tp_pct = config.getfloat(section, 'med_vol_tp_pct', fallback=12.0)
            self.high_vol_tp_pct = config.getfloat(section, 'high_vol_tp_pct', fallback=18.0)
            self.resistance_lookback_bars = config.getint(section, 'resistance_lookback_bars', fallback=50)
            self.resistance_margin_pct = config.getfloat(section, 'resistance_margin_pct', fallback=1.0)
            self.min_tp_pct = config.getfloat(section, 'min_tp_pct', fallback=6.0)
            self.max_tp_pct = config.getfloat(section, 'max_tp_pct', fallback=25.0)

            # ===== RISK MANAGEMENT (WorkerStopManager) =====
            self.stop_manager = create_worker_stop_manager(config, section)

        else:
            # Fallback defaults si no hay config
            self.min_price = 1.0
            self.max_price = 15.0
            self.min_quality_score = 40.0
            self.min_price_above_vwap_pct = 1.0
            self.min_vwap_slope = 0.0001
            self.max_rsi_entry = 75.0

            # Dual mode
            self.strong_trend_threshold_pct = 2.0

            self.low_vol_atr_threshold = 3.0
            self.low_vol_dip_min = 1.0
            self.low_vol_dip_max = 3.0

            self.med_vol_atr_threshold = 6.0
            self.med_vol_dip_min = 3.0
            self.med_vol_dip_max = 5.0

            self.high_vol_dip_min = 5.0
            self.high_vol_dip_max = 8.0

            self.min_bounce_bars = 2
            self.min_bounce_volume_ratio = 1.2

            self.max_wait_minutes = 60
            self.max_entry_attempts = 2

            # Dynamic TP defaults
            self.low_vol_tp_pct = 8.0
            self.med_vol_tp_pct = 12.0
            self.high_vol_tp_pct = 18.0
            self.resistance_lookback_bars = 50
            self.resistance_margin_pct = 1.0
            self.min_tp_pct = 6.0
            self.max_tp_pct = 25.0

            from .worker_stop_manager import WorkerStopManager, WorkerStopConfig
            self.stop_manager = WorkerStopManager(WorkerStopConfig(
                stop_loss_pct=3.0,
                take_profit_pct=12.0,  # Base TP (se ajustará dinámicamente)
        ...
        self.logger.info(f"   DUAL MODE: Strong Trend >{self.strong_trend_threshold_pct}% -> Direct Entry, <{self.strong_trend_threshold_pct}% -> Wait Dip")
        ...
        self.logger.info(f"   Bounce: {self.min_bounce_bars} bars, {self.min_bounce_volume_ratio}x volume")
    ...
    async def should_enter(self, opportunity: Dict) -> bool:
        ...
        try:
            ...
            # =================
            # FILTER: TIME CHECK (Standardized - ADDED)
            # =================
            ...
            # =================
            # FILTER: ODS (Standardized - ADDED)
            # =================
            # We need bars for ODS, so we fetch them early if possible or defer
            # Bars are fetched in STEP 1 below, let's fetch them now to allow ODS check
            bars = self.get_bars_from_opportunity(opportunity)
            
            # Re-using bars variable in Step 1
            ...
            # Check duplicate positions (via UnifiedPositionManager)
            ...
            if unified_manager and unified_manager.is_symbol_blocked(symbol):
                ...
                self.logger.warning(f"⚪ {symbol}: BLOCKED - already held in {strategy_type.upper()} trading")
                return False
            ...
            # ====
            # STEP 5.5: EARLY RESISTANCE VALIDATION (NEW)
            # ====
            # Check if there's a nearby resistance that would make R:R impossible
            # This prevents entering trades that will be rejected later by structural_exit_calculator
            ...
            if distance_to_resistance < early_validation_threshold:
                # Estimate TP at 80% of distance to resistance (with buffer)
                estimated_tp_pct = distance_to_resistance * 0.8

                # Typical SL for buy_the_dip based on volatility
                estimated_sl_pct = dip_analysis.atr_pct * 1.5  # 1.5x ATR as typical SL
