...
class AutowhlrLogic(BaseWorkerLogic):
    """
    Auto-Generated Worker by WorkerGen AI.
    Strategy Score: 50.65
    """
    ...
    def __init__(self, worker_name, config, execution_engine, risk_manager, event_queue):
        ...
        self.stop_loss_pct = 3.93
        self.take_profit_pct = 5.16
        self.time_limit_bars = 311
    ...
    async def should_enter(self, opportunity):
        ...
        # Logic: high < ema_20 AND close <= ema_9 AND volume > open
        
        if not (current['high'] < current['ema_20']):
            # Fail: high < ema_20
            return False
        if not (current['close'] <= current['ema_9']):
            # Fail: close <= ema_9
            return False
        if not (current['volume'] > current['open']):
            # Fail: volume > open
            return False
