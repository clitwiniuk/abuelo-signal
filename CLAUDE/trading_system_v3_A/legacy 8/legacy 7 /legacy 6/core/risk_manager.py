...
class RiskManager(IRiskManager):
    ...
    def _check_daily_loss_limit(self) -> bool:
        ...
        # Calculate dynamic limit based on percentage if available
        if hasattr(self.config, 'max_daily_loss_pct') and hasattr(self.config, 'portfolio_capital'):
            # Calculate limit: e.g., 2000 * 5.0 / 100 = 100 -> limit is -100
            limit_amount = self.config.portfolio_capital * (self.config.max_daily_loss_pct / 100.0)
            max_daily_loss = -abs(limit_amount) # Ensure it's negative
        else:
            # Fallback to fixed value or default
            max_daily_loss = getattr(self.config, 'max_daily_loss', -100.0)
            
        # Check if current PnL is below the limit (more negative)
        # Example: PnL -110 < Limit -100 -> True (Exceeded)
        # We return True if SAFE (PnL > Limit), False if EXCEEDED
        
        is_safe = self.daily_pnl > max_daily_loss
        
        if not is_safe:
            self.logger.warning(f"⚠️ Daily Loss Limit Exceeded: PnL ${self.daily_pnl:.2f} < Limit ${max_daily_loss:.2f} ({getattr(self.config, 'max_daily_loss_pct', 'N/A')}%)")
