#!/usr/bin/env python3
"""
VCP Smallcap Worker - Diagnostic Analysis

Analyzes why the VCP worker is not finding trades with real data.
Logs detailed rejection reasons at each filter stage.
"""
...
class VCPDiagnostics:
    ...
    async def analyze_symbol(self, symbol: str, days: int = 7):
        ...
        for day, day_bars in bars_by_day.items():
            ...
            evaluation_points = [60, 120, 180, 240, 300]  # Different bar indices
...
async def main():
    # Use same symbols as momentum analysis for comparison
    symbols = ['CRCG', 'BTBT', 'DEFT', 'RR', 'WRD', 'VEEE', 'NVTS', 'ESPR', 'MSTX', 'JBLU']
    
    diagnostics = VCPDiagnostics()
    diagnostics.connect()
    
    print("\n" + "="*80)
    print("🔬 VCP SMALLCAP WORKER - DIAGNOSTIC ANALYSIS")
    print("="*80)
