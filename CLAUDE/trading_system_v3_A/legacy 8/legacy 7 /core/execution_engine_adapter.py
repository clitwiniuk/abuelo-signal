...
class ExecutionEngineAdapter:
    ...
    async def enter_position(
        ...
        async with self._position_locks[symbol]:
            ...
            try:
                ...
                # SIMULATION/PAPER MODE DETECTION: Skip IBKR wait loops
                if self._is_simulation_mode():
                    self.logger.info(f"📄 {strategy}: Paper/Simulation mode detected - bypassing execution confirmation")
                    execution_confirmed = True
                else:
                    # Wait for execution confirmation (Live/Real environment)
                ...
                # SIMULATION/PAPER MODE DETECTION: Skip IBKR position wait
                if self._is_simulation_mode():
                    self.logger.debug(f"📄 {strategy}: Paper/Simulation mode detected - using simulated price")
    ...
    async def exit_position(
        ...
        try:
            ...
            # ============================================================
            # 🎯 SIMPLE & RELIABLE: Get actual exit price
            # ============================================================
            ...
            # SIMULATION/PAPER MODE DETECTION: Skip wait loops
            if self._is_simulation_mode():
                self.logger.info(f"📄 {symbol}: Paper/Simulation mode detected - bypassing exit confirmation")
    ...
    def _is_simulation_mode(self) -> bool:
        """
        Detects if the current environment is simulation, paper trading, or mock.
        This is used to bypass real-time confirmation loops that never fill in simulation.
        """
        if not self.broker:
            return True
        ...
        # Check for common mock/simulation class names
        if any(name in broker_name for name in ['Mock', 'Replay', 'Simulation', 'Backtest', 'Test']):
            return True
            
        # Check for explicit flags
        if hasattr(self.broker, 'paper_trading_mode') and self.broker.paper_trading_mode:
            return True
        if hasattr(self.broker, 'is_paper_trading') and self.broker.is_paper_trading:
            return True
        if hasattr(self.broker, 'simulation_mode') and getattr(self.broker, 'simulation_mode', False):
            return True
