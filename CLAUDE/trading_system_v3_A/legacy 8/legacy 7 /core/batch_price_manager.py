...
class BatchPriceManager:
    ...
    def __init__(self, ibkr_adapter):
        ...
        self.price_timestamps: Dict[str, datetime] = {}  # symbol -> last update timestamp
            ...
            try:
                ...
                if price and price > 0:
                    ...
                    with self.subscription_lock:
                        ...
                        self.price_timestamps[symbol] = current_time
    ...
    def get_current_price_with_timestamp(self, symbol: str) -> Tuple[float, datetime]:
        """
        Get precio actual con timestamp de última actualización

        Returns:
            Tuple[float, datetime]: (price, last_update_timestamp)
            Si no hay precio, retorna (0.0, None)
        """
        with self.subscription_lock:
            price = self.last_prices.get(symbol, 0.0)
            timestamp = self.price_timestamps.get(symbol, None)
    ...
    def is_price_fresh(self, symbol: str, max_age_seconds: int = 5) -> bool:
        """
        Check if price is fresh (within specified age limit)

        Args:
            symbol: Stock symbol
            max_age_seconds: Maximum age in seconds (default: 5 seconds)
        ...
        if not is_fresh:
            logger.debug(f"⚠️ Price for {symbol} is stale: {age_seconds:.1f}s old (max: {max_age_seconds}s)")
    ...
    async def remove_symbol_subscription(self, symbol: str) -> bool:
        ...
        try:
            with self.subscription_lock:
                ...
                # Cancel market data subscription (ignore if reqId not found)
                ticker = self.active_subscriptions[symbol]
                try:
                    self.ibkr.ib.cancelMktData(ticker.contract)
                except Exception as e:
                    # Ignore "No reqId found" errors - subscription may have already been cancelled
                    logger.debug(f"📡 Note: {symbol} subscription already cancelled or not found")
                ...
                try:
                    ...
                    if ticker:
                        try:
                            ...
                            # Ignore "No reqId found" errors during cleanup
                            logger.debug(f"📡 {symbol} subscription already cancelled")
                            cleanup_count += 1
    ...
    def get_status_report(self) -> str:
        ...
        with self.subscription_lock:
            ...
            fresh_prices = len([s for s in self.active_subscriptions.keys() if self.is_price_fresh(s, 5)])

        report = [
            "📊 BATCH PRICE MANAGER STATUS",
            f"📡 Active subscriptions: {active_count}",
            f"💰 Prices available: {prices_available}",
            f"⚡ Fresh prices (<5s): {fresh_prices}",
