...
class VCPSmallcapWorkerLogic(BaseWorkerLogic):
    ...
    def __init__(self, worker_name, broker, risk_manager=None, config=None, execution_engine=None):
        super().__init__(
            worker_name=worker_name,
            broker=broker,
            config=config
        )
        self.risk_manager = risk_manager # Store locally if needed, though risk checks might be centralized
    ...
    async def calculate_pattern_completion(self, opportunity: Dict[str, Any]) -> Tuple[float, float]:
        ...
        PURELY TECHNICAL IMPLEMENTATION - No metadata shortcuts.
        """
        try:
            symbol = opportunity.get('symbol', 'UNKNOWN')
            current_price = opportunity.get('current_price', 0)
            
            # Use standardized BarData bars from BaseWorkerLogic
            bars = self.get_bars_from_opportunity(opportunity)
            
            if not bars:
                return 0.0, 0.0
            
            # No metadata shortcuts (Supply Exhaustion, Blue Sky)
    ...
    async def _calculate_vcp_completion(
        ...
        current_price: float,
        symbol: str,
        allow_flat_structure: bool = False # Deprecated, kept for signature compatibility but ignored
    ) -> Tuple[float, float]:
        """
        Calcula % de completitud del patrón VCP (0-100%) PURELY TECHNICAL
        
        STRICT VCP DEFINITION:
        1. 2+ Contractions
        2. Contracting volatility (ranges get smaller)
        3. Contracting volume (volume dries up)
        4. Close to pivot point
        """
        try:
            if not bars or len(bars) < 20:
                return 0.0, 0.0

            # 1. Detect Contractions
            contractions = self._detect_contractions(bars)
            num_contractions = len(contractions)
            
            min_required = self.min_contractions
            
            if num_contractions < min_required:
                return 20.0, 0.0 # Basic data ok but structure missing

            # 2. Verify Volatility Contraction (Ranges must decrease)
            # Strict mode: Each contraction range should be smaller than previous (or roughly same)
            # We allow a small tolerance factor (1.1x) to not be overly rigid
            ranges_decreasing = True
            debug_ranges = []
            
            if num_contractions >= 2:
                last_range = contractions[0]['range_pct']
                debug_ranges.append(last_range)
                for i in range(1, len(contractions)):
                    curr_range = contractions[i]['range_pct']
                    debug_ranges.append(curr_range)
                    if curr_range > (last_range * 1.1): # Allow 10% tolerance
                         ranges_decreasing = False
                    last_range = curr_range

            if not ranges_decreasing:
                return 40.0, 0.0 # Structure found but volatility expanding/flat

            # 3. Verify Volume Contraction
            volume_valid = self._validate_volume_pattern(contractions)
            if not volume_valid:
                 return 50.0, 0.0 # Volatility good, volume bad

            # 4. Check Proximity to Pivot (High of last contraction)
            ...
            # Support level for stop loss (Low of last contraction)
            support_level = last_c['low']

            if -0.5 <= dist_pct <= 2.0: # Close to pivot (within 2% below or 0.5% above)
                return 95.0, support_level # READY FOR ENTRY
            elif dist_pct <= 5.0:
                return 80.0, support_level # Forming well
            
            return 60.0, support_level

        except Exception as e:
            self.logger.error(f"Error calculating VCP completion: {e}")
            return 0.0, 0.0
