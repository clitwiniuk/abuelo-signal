...
class WorkerStopManager:
    ...
    def check_exit(
        ...
        # Check if we are past the EOD exit time
        # CRITICAL FIX: Don't restrict to regular hours. If it's 20:00 (Extended Close), we MUST exit too.
        # We just check if current_time >= exit_time.
        
        is_past_eod_time = current_hour >= self.config.end_of_day_hour
        
        # If we are in Extended Hours (e.g. 19:00) and EOD was 15:58, we should definitely exit.
