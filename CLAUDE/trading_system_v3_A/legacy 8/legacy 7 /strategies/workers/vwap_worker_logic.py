...
class VWAPWorkerLogic(BaseWorkerLogic):
    ...
    def __init__(self, broker, risk_manager=None, config=None, execution_engine=None):
        ...
        section = 'VWAP_BREAKOUT_STRATEGY'

        self.min_volume_ratio = get_cfg(section, 'min_volume_ratio', 1.5)
        self.min_daily_volume = get_cfg(section, 'min_daily_volume', 500_000, int)
        self.vwap_window_minutes = get_cfg(section, 'vwap_window_minutes', 120, int)
        self.rsi_period = get_cfg(section, 'rsi_period', 14, int)
        self.rsi_overbought = get_cfg(section, 'rsi_overbought', 85)
        self.rsi_oversold = get_cfg(section, 'rsi_oversold', 15)
        self.min_price = get_cfg(section, 'min_price', 2.0)
        self.max_price = get_cfg(section, 'max_price', 50.0)

        # CRITICAL: Minimum VWAP trend required for breakout (from config.ini)
        self.min_vwap_trend_pct = get_cfg(section, 'min_vwap_trend_pct', 0.08)
        ...
        try:
            ...
            try:
                ...
                if hasattr(self.broker, 'get_bars'):
                    bars = await self.broker.get_bars(symbol, '1 min', 120)
                    if bars:
            ...
            if not should_exit and market_data and 'bars' in market_data:
