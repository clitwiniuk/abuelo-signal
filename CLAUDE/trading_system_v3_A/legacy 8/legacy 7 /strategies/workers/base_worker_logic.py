...
class BarDataWrapper:
    ...
    def __init__(self, bar_dict: Dict[str, Any]):
        ...
        # Extended fields for parity with ReplayBar and Worker needs
        self.rsi = bar_dict.get('rsi')
        self.macd = bar_dict.get('macd')
        self.macd_signal = bar_dict.get('macd_signal')
        self.vwap = bar_dict.get('vwap')
        self.volume_ratio = bar_dict.get('volume_ratio', 0.0)
        self.gap_percentage = bar_dict.get('gap_percentage', 0.0)
        self.symbol = bar_dict.get('symbol') # Ensure symbol is available if in dict
...
class BaseWorkerLogic(ABC):
    ...
    def __init__(
        ...
        Args:
            ...
            broker: Broker Adapter (Live o Simulado)
        ...
        # ============================================================================
        # ACTIVE SURVEILLANCE (Phase 1)
        # ============================================================================
        # Candidates to watch for potential entry (Scanner "Near Misses")
        # {symbol: {'opportunity': dict, 'expiry_time': datetime}}
        self.candidates: Dict[str, Dict[str, Any]] = {}
        
        # Surveillance Configuration
        surveillance_cfg = configparser.ConfigParser() if hasattr(self, 'config_parser') else None
        # Try to find SURVEILLANCE section in config object or parser
        self.surveillance_enabled = True
        self.watch_time_minutes = 30 # Default (increased from 15)
        self.max_candidates = 50     # Default (increased from 10)
        self.watch_after_exit_minutes = 390 # Default (EOD)
        
        try:
            # Try to load from unified config object attribute if available
            if hasattr(self.config, 'candidate_watch_time_minutes'):
                self.watch_time_minutes = int(self.config.candidate_watch_time_minutes)
            
            # If using ConfigParser (v2.0 workers)
            if hasattr(self, 'config_parser') and self.config_parser and self.config_parser.has_section('SURVEILLANCE'):
                self.watch_time_minutes = int(self.config_parser.get('SURVEILLANCE', 'candidate_watch_time_minutes', fallback=30))
                self.max_candidates = int(self.config_parser.get('SURVEILLANCE', 'max_candidates_per_worker', fallback=50))
                self.watch_after_exit_minutes = int(self.config_parser.get('SURVEILLANCE', 'watch_after_exit_minutes', fallback=390))
        except Exception as e:
            self.logger.warning(f"⚠️ Failed to load surveillance config: {e}")

        self.logger.info(f"🔧 Worker {worker_name} initialized with Active Surveillance (Watch {self.watch_time_minutes}m, Max {self.max_candidates})")
    ...
    async def _restore_active_positions(self):
        ...
        try:
            ...
            # Unified Source via Broker Adapter
            all_positions = await self.broker.get_all_positions()
            
            for pos in all_positions:
                # In Live (IBKR), broker might return positions for ALL strategies
                # We need to filter for this worker if metadata is available, 
                # or "claim" orphaned positions if configured.
                
                # For now, simplistic restore
                if hasattr(pos, 'account'): # Verify it's a valid position object
                     symbol = pos.symbol
                     
                     # Check if we should manage this position
                     # TODO: Add strategy metadata to BrokerPosition to filter correctly
                     # For Replay: We only get what we traded
                     restored_count += 1
                     self._restore_position_from_broker(symbol, pos)
    ...
    def _restore_position_from_broker(self, symbol: str, broker_position: Any):
        """Restore position from Broker (fallback)"""
        # Extract data from BrokerPosition object
        # BrokerPosition has standard fields: avg_cost, quantity
        entry_price = getattr(broker_position, 'avg_cost', 0.0)
        quantity = getattr(broker_position, 'quantity', 0.0)
    ...
    async def run(self):
        ...
        try:
            while self.is_running:
                ...
                # ACTIVE SURVEILLANCE
                try:
                    await self._monitor_candidates()
                except Exception as e:
    ...
    async def process_opportunity(self, opportunity: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if entry_approved:
                ...
                # ACTIVE SURVEILLANCE: Watch near-misses
                # If rejected but has decent quality, watch it!
                if hasattr(self, 'surveillance_enabled') and self.surveillance_enabled:
                    quality = float(opportunity.get('quality_score', 0))
                    min_q = float(getattr(self.config, 'min_quality_score_for_watch', 60.0))
                    
                    if quality >= min_q:
                        self.logger.info(f"👀 High Quality Rejection (Q={quality}): Adding to surveillance")
                        await self.watch_candidate(symbol, opportunity, duration_minutes=self.watch_time_minutes)
    ...
    async def watch_candidate(self, symbol: str, opportunity: Dict[str, Any], duration_minutes: int):
        """
        Add a candidate to the active surveillance list
        """
        if len(self.candidates) >= self.max_candidates:
            # Drop oldest
             oldest = min(self.candidates.items(), key=lambda x: x[1]['expiry_time'])
             del self.candidates[oldest[0]]

        expiry = datetime.now() + timedelta(minutes=duration_minutes)
        self.candidates[symbol] = {
            'opportunity': opportunity,
            'expiry_time': expiry,
            'last_price': opportunity.get('current_price', 0)
        }
        self.logger.info(f"👀 WATCHING {symbol} for {duration_minutes}m (Expires: {expiry.strftime('%H:%M:%S')})")
        
        # Ensure we are subscribed to realtime data
        if hasattr(self.broker, 'batch_price_manager'):
            await self.broker.batch_price_manager.subscribe_to_positions([symbol])

    async def _monitor_candidates(self):
        """
        Monitor watched candidates for entry signals
        """
        ...
        expired = []
         ...
         # Use list() to allow modification during iteration if needed (though we collect expiries first)
        for symbol, data in list(self.candidates.items()):
            # 1. Check Expiry
            ...
            # Try efficient batch manager first
            current_price = 0.0
            if hasattr(self.broker, 'batch_price_manager'):
                current_price = self.broker.batch_price_manager.get_price(symbol)
            
            # Fallback to sync call if needed (but prefer not to block)
            if current_price <= 0:
                # Skip this tick if no price data
                continue
            ...
            # 3. Re-evaluate Entry
            # We construct an updated opportunity object with the NEW price
            # This is crucial: we use the ORIGINAL opportunity data (catalysts, etc) 
            # but updated with current market data
            opp = data['opportunity'].copy()
            opp['current_price'] = current_price
            # TODO: Update other realtime metrics if possible (volume, RSI)
            # For now, price action is the main trigger for limit orders/breakouts
            
            try:
                should_enter = await self.should_enter(opp)
                
                if should_enter:
                    self.logger.info(f"⚡ SURVEILLANCE TRIGGERED: {symbol} @ ${current_price:.2f}")
                    # Execute Entry
                    success = await self._execute_entry(opp)
                    if success:
                        # Remove from candidates if successfully entered
                        # (It moves to active_positions)
                        if symbol in self.candidates:
                            del self.candidates[symbol]
            except Exception as e:
                self.logger.error(f"Error evaluating candidate {symbol}: {e}")

        # Clean up expired
        for sym in expired:
            if sym in self.candidates:
                del self.candidates[sym]
                self.logger.debug(f"❌ Stopped watching {sym} (Expired)")
    ...
    async def _execute_entry(self, opportunity: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            # Prepare Order
            from core.interfaces.broker_adapter import BrokerOrder
            
            # Use RiskManager for sizing if available (or calculate locally)
            # For this refactor, we assume RiskManager is still used for *calculation* but not execution
            # Or simplified: Scanner/Worker separation means we just buy X amount? 
            # Ideally: risk_manager.calculate_size(account, price, sl) -> qty
            
            # Simple assumption for V3 refactor:
            # We calculate quantity here or use RiskManager in a standalone way (not integrated in ExecutionEngine)
            quantity = 100 # DEFAULT PLACEHOLDER - TODO: Hook up Sizing Logic
            if hasattr(self, 'risk_manager') and hasattr(self.risk_manager, 'calculate_position_size'):
                 # We still have risk_manager, try to use it for sizing only
                 # Or use simple sizing: $5000 / price
                 try:
                     # This depends on risk_manager API. Assuming simplified:
            ...
            order = BrokerOrder(
                ...
                action='BUY',
                order_type='MKT',
                algo_strategy=self.worker_name
            )
            ...
            if trade_result:
                ...
                # Manual Position Tracking (since Broker is async/stateless)
                # In Backtrader, we get a notification. Here we assume immediate fill for simplicity or polling
                # For Live, place_order returns trade object.
                
                # Approximate fill price if simplified
                fill_price = current_price 
                if hasattr(trade_result, 'avgFillPrice'): fill_price = trade_result.avgFillPrice
                ...
                self.logger.info(
                    ...
                    f"{symbol} @ ${position_data.get('entry_price', 0):.2f} "
                    f"x {position_data.get('quantity', 0)} shares "
    ...
    async def _get_current_price(self, symbol: str) -> float:
        """
        Obtiene precio actual del mercado vía Broker
        """
    ...
    async def _execute_exit(self, symbol: str, reason: str, current_price: float):
        ...
        try:
            ...
            # Ejecutar cierre usando Broker Adapter
            from core.interfaces.broker_adapter import BrokerOrder
            ...
            if quantity > 0:
                ...
                self.logger.warning(f"⚠️ {symbol}: Quantity is 0, nothing to exit")
                return
            ...
            # ACTIVE SURVEILLANCE: Keep watching after exit! (As per user request)
            if hasattr(self, 'surveillance_enabled') and self.surveillance_enabled:
                # Reconstruct opportunity from stored data or create minimal one
                opp = position_data.get('opportunity_data', {})
                opp['symbol'] = symbol
                opp['current_price'] = float(current_price or 0.0) # Ensure float
                # TODO: Retrieve more info if needed

                # Watch until End of Day (default 390 mins or config)
                self.logger.info(f"👀 Cycle complete. Re-adding {symbol} to surveillance (Post-Trade Monitoring)")
                await self.watch_candidate(symbol, opp, duration_minutes=self.watch_after_exit_minutes)
