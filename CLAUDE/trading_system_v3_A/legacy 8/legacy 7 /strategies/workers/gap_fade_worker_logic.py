...
class GapFadeWorkerLogic(BaseWorkerLogic):
    ...
    def __init__(self, broker, risk_manager=None, config=None, execution_engine=None):
        # Initialize with worker_name, broker, and risk_manager
        super().__init__(
        ...
        # Use config object if available, otherwise defaults
        if config:
            # Section in config.ini
            section = 'GAP_FADE_STRATEGY'
            
            # Helper to safely get from config object or dict
            def get_cfg(key, default, type_func=None):
                try:
                    # 1. Check direct attributes (MockConfig support)
                    if hasattr(config, key):
                        val = getattr(config, key)
                        if val is not None: return val
                    
                    # 2. Check overrides (MockConfig support)
                    if hasattr(config, 'overrides') and key in config.overrides:
                        return config.overrides[key]
                        
                    # 3. Standard configparser access
                    if hasattr(config, 'get'):
                        if type_func == 'float' and hasattr(config, 'getfloat'):
                            return config.getfloat(section, key, fallback=default)
                        if type_func == 'int' and hasattr(config, 'getint'):
                            return config.getint(section, key, fallback=default)
                        if type_func == 'bool' and hasattr(config, 'getboolean'):
            ...
            self.min_gap_percent = get_cfg('gap_fade_min_gap_pct', 5.0, 'float')
            self.max_catalyst_strength = get_cfg('gap_fade_max_catalyst_strength', 6, 'int')
            self.gap_failure_threshold = get_cfg('gap_fade_failure_threshold', 0.75, 'float')
            self.volume_decline_threshold = get_cfg('gap_fade_volume_decline_pct', 0.50, 'float')
            self.entry_window_start = get_cfg('gap_fade_entry_start', '10:00')
            self.entry_window_end = get_cfg('gap_fade_entry_end', '11:00')
            self.conservative_target_pct = get_cfg('gap_fade_conservative_target', 0.50, 'float')
            self.aggressive_target_pct = get_cfg('gap_fade_aggressive_target', 1.0, 'float')
            self.exit_time = get_cfg('gap_fade_exit_time', '15:45')
            self.stop_above_hod_buffer = get_cfg('gap_fade_stop_buffer_pct', 1.5, 'float')
            self.trailing_activation_pct = get_cfg('gap_fade_trailing_activation', 3.0, 'float')
            self.min_quality_score = get_cfg('gap_fade_min_quality_score', 6.0, 'float')
            self.min_float = get_cfg('gap_fade_min_float_millions', 50.0, 'float')
            self.min_adv = get_cfg('gap_fade_min_adv_millions', 1.0, 'float')
            self.max_short_interest = get_cfg('gap_fade_max_si_pct', 20.0, 'float')
            self.vwap_slope_threshold = get_cfg('gap_fade_vwap_slope_threshold', -0.10, 'float')
        
        else:
            # Defaults
            self.min_gap_percent = 5.0
            self.max_catalyst_strength = 6
            self.gap_failure_threshold = 0.75
            self.volume_decline_threshold = 0.50
            self.entry_window_start = '10:00'
            self.entry_window_end = '11:00'
            self.conservative_target_pct = 0.50
            self.aggressive_target_pct = 1.0
            self.exit_time = '15:45'
            self.stop_above_hod_buffer = 1.5
            self.trailing_activation_pct = 3.0
            self.min_quality_score = 6.0
            self.min_float = 50.0
            self.min_adv = 1.0
            self.max_short_interest = 20.0
            self.vwap_slope_threshold = -0.10
    ...
    def evaluate_opportunity(self, opportunity: Dict) -> bool:
        ...
        self.logger.info(f"   Timestamp: {opportunity.get('timestamp')}")
        self.logger.info(f"   Price: {opportunity.get('current_price')}")
        self.logger.info(f"   Gap %: {opportunity.get('gap_percentage')}")
        ...
        can_short, short_reason = can_enter_short(timestamp)
        if not can_short:
        ...
        # Check entry window
        if not self._is_within_entry_window(timestamp):
        ...
        if float_shares and float_shares > 0:
            ...
            self.logger.debug(f"ℹ️ {symbol}: float_shares not provided, skipping float filter")
    ...
    def _is_within_entry_window(self, timestamp: Optional[datetime] = None) -> bool:
        """Check if given time is within entry window (10:00-11:00 AM)"""
        try:
            if timestamp is None:
                # Get current ET time (for live trading)
                from core.market_hours import get_market_hours
                now_dt = get_market_hours().get_current_et_time()
            else:
                now_dt = timestamp
    ...
    def _analyze_gap(self, bars: list, opportunity: Dict) -> Dict:
        ...
        try:
            ...
            if not previous_close or not open_price:
                # FALLBACK for simulations: Use gap_percentage if provided
                gap_pct = opportunity.get('gap_percentage')
                if gap_pct is not None:
                    # We can't know gap_range exactly without prices, 
                    # but we can estimate or use a dummy if prices missing
                    # Actually, if we have current_price we can estimate previous_close
                    curr_price = opportunity.get('current_price')
                    if curr_price and gap_pct != 0:
                        # gap_pct = (curr - prev) / prev * 100 -> prev = curr / (1 + gap/100)
                        # This assumes curr_price is close to open_price
                        est_prev = curr_price / (1 + gap_pct / 100)
                        return {
                            'has_gap': abs(gap_pct) >= self.min_gap_percent,
                            'gap_pct': gap_pct,
                            'previous_close': est_prev,
                            'open_price': curr_price,
                            'gap_range': curr_price - est_prev
                        }
    ...
    async def should_exit(self, symbol: str, position: Dict, current_price: float) -> tuple[bool, str]:
        ...
        try:
            ...
            # Time-based exit (15:45 mandatory close ET)
            # Use current_price's associated time if available in bars_history
            bars = position.get('bars_history', [])
            if not bars:
                # If no bars, we can't accurately determine simulated time
                # Fall back to base class or current ET
                from core.market_hours import get_market_hours
                now_dt = get_market_hours().get_current_et_time()
            else:
                last_bar_time = bars[-1].get('timestamp')
                if isinstance(last_bar_time, str):
                    now_dt = datetime.fromisoformat(last_bar_time)
                else:
                    now_dt = last_bar_time

            exit_hour, exit_min = map(int, self.exit_time.split(':'))
            exit_time_today = now_dt.replace(hour=exit_hour, minute=exit_min, second=0)

            if now_dt >= exit_time_today:
                return True, f"TIME_STOP_{self.exit_time}"
