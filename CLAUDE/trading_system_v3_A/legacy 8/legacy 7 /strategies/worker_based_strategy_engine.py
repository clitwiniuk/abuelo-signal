...
class WorkerBasedStrategyEngine:
    ...
    def __init__(
        ...
        Args:
            broker: Broker Adapter (Live or Simulated)
        ...
        #Get config from execution_engine if available, or just use provided config
        # Now we don't have execution_engine, so we rely on provided config or defaults
        ...
        try:
            ...
            if daily_plays_enabled:
                self.workers['daily_plays'] = DailyPlaysWorkerLogic(
                    execution_engine=None, # Legacy deprecated
                    broker=self.broker,
                    risk_manager=self.risk_manager,
                    config=self.config_parser  # NOW PASSING ConfigParser (v2.0 refactored)
                )
                self.workers['daily_plays'].broker = self.broker # Ensure injection
