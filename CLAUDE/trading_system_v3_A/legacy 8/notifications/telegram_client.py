...
🔍 **SCANNER & PLAYS**
• /plays - Plays detectados (activos)
• /scanner - Estado del scanner

📋 **LOGS**
• /scanner_log - Scanner logs de hoy
• /trader_log - Trader logs de hoy

🤖 **WORKERS**
• /workers - Estado de workers
• /position &lt;symbol&gt; - Detalles de posición
...
• `/performance` - Performance summary (7 days)
• `/trades_today` - Today's trades
• `/trades_open` - Open positions
• `/closeall` - 🚨 Emergency close all positions
• `/plays` - Active plays detected
• `/scanner` - Scanner status
• `/scanner_log` - Scanner logs (today)
• `/trader_log` - Trader logs (today)
• `/workers` - Workers status
• `/position AAPL` - Position details for AAPL
...
def send_eod_summary():
    """Send End of Day Summary (Intraday vs Swing)"""
    if not is_enabled():
        return
    ...
    try:
        ...
        if not db_path.exists():
            send_message("❌ Database not found for EOD summary")
            return
        ...
        with sqlite3.connect(db_path) as conn:
            ...
            # Get all trades for today (or open swing trades from before)
            cursor.execute("""
                SELECT symbol, status, pnl, entry_time, exit_time, strategy 
                FROM trades 
                WHERE DATE(entry_time) = ? OR status = 'OPEN'
            """, (today,))
        ...
        for row in rows:
            is_swing = 'swing' in (row['strategy'] or '').lower() or 'squeeze' in (row['strategy'] or '').lower()
            status = row['status']
            pnl = row['pnl'] if row['pnl'] else 0.0
            
            if status == 'CLOSED':
                total_pnl += pnl
                if is_swing:
                    swing_closed.append(row)
                else:
                    intraday_closed.append(row)
            elif status == 'OPEN':
                if is_swing:
                    swing_open.append(row)
                # Intraday open at EOD shouldn't happen usually, but if so, list it?
                # For now focus on Swing Open
        ...
        msg += f"📉 **INTRADAY (Closed):**\n"
        if intraday_closed:
            intra_pnl = sum(r['pnl'] for r in intraday_closed if r['pnl'])
            msg += f"• P&L: ${intra_pnl:+.2f} ({len(intraday_closed)} trades)\n"
            for t in intraday_closed:
                icon = "🟢" if (t['pnl'] or 0) > 0 else "🔴"
                msg += f"   {icon} {t['symbol']}: ${t['pnl']:+.2f}\n"
        else:
            msg += "• No intraday trades today\n"
            
        msg += "\n"
        
        # 2. Swing Positions (Open)
        msg += f"🐻 **SWING (Open):**\n"
        if swing_open:
            msg += f"• Active Positions: {len(swing_open)}\n"
            for t in swing_open:
                entry_date = t['entry_time'][:10]
                msg += f"   ⏳ {t['symbol']} (Entry: {entry_date})\n"
        else:
            msg += "• No active swing positions\n"
            
        msg += "\n"
        
        # 3. Total
        msg += f"💰 **TOTAL REALIZED P&L:** ${total_pnl:+.2f}\n"
        msg += f"═══════════════════════"
        
        send_message(msg, parse_mode="Markdown")
        _logger.info("Sent EOD summary to Telegram")

    except Exception as e:
        _logger.error(f"Error sending EOD summary: {e}")
        send_message(f"❌ Error sending EOD summary: {e}")
...
def send_workers_status():
    """Send status of all workers and their positions"""
    if not is_enabled():
        return
    ...
    try:
        ...
        if not unified_system or not hasattr(unified_system, 'strategy_engine'):
            send_message("❌ Strategy engine not available")
            return
        
        strategy_engine = unified_system.strategy_engine
        
        if not hasattr(strategy_engine, 'workers'):
            send_message("❌ Workers not found in strategy engine")
            return
        ...
        for worker_name, worker in strategy_engine.workers.items():
            if hasattr(worker, 'active_positions'):
                positions = worker.active_positions
                position_count = len(positions)
                total_positions += position_count
                
                if position_count > 0:
                    active_workers += 1
                    symbols = ', '.join(positions.keys())
                    workers_info.append(f"✅ **{worker_name}**: {position_count} pos ({symbols})")
                else:
                    workers_info.append(f"⚪ {worker_name}: 0 pos")
        
        # Format message
        workers_text = "\n".join(workers_info) if workers_info else "No workers found"
        
        status_msg = f"""🤖 **WORKERS STATUS**
═══════════════════════════════

📊 **Summary:**
• Total workers: {len(strategy_engine.workers)}
...
⏰ {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"""
        ...
        send_message(status_msg, use_html=True)
        _logger.info(f"Sent workers status: {len(strategy_engine.workers)} workers, {total_positions} positions")
        
    except Exception as e:
        _logger.error(f"Error getting workers status: {e}")
        send_message(f"❌ Error obteniendo estado de workers: {str(e)}")

def send_position_details(symbol: str):
    """Send detailed information for a specific position"""
    if not is_enabled():
        return
    ...
    try:
        ...
        if not unified_system or not hasattr(unified_system, 'strategy_engine'):
            send_message(f"❌ Strategy engine not available for {symbol}")
            return
        
        strategy_engine = unified_system.strategy_engine
        
        # Find position in workers
        position_found = False
        worker_name = None
        position_data = None
        
        if hasattr(strategy_engine, 'workers'):
            for wname, worker in strategy_engine.workers.items():
                if hasattr(worker, 'active_positions') and symbol in worker.active_positions:
                    position_found = True
                    worker_name = wname
        ...
        if not position_found:
            send_message(f"ℹ️ No active position found for **{symbol}**\n\nUse /trades_open to see all open positions.")
            return
        ...
        if db_path.exists():
            try:
                with sqlite3.connect(db_path) as conn:
                    cursor = conn.cursor()
                    cursor.execute("""
                        SELECT entry_time, take_profit_pct, stop_loss_pct
                        FROM trades
                    ...
                    result = cursor.fetchone()
                    if result:
                        entry_time_str = result[0] or "Unknown"
                        take_profit_pct = result[1] or 0
                        stop_loss_pct = result[2] or 0
            except Exception as db_error:
                _logger.warning(f"Could not get DB data for {symbol}: {db_error}")
        ...
        if entry_time_str != "Unknown" and len(entry_time_str) >= 19:
            try:
                from datetime import datetime
        ...
        # Calculate targets
        tp_price = entry_price * (1 + take_profit_pct / 100) if take_profit_pct > 0 else 0
        sl_price = entry_price * (1 - stop_loss_pct / 100) if stop_loss_pct > 0 else 0
        
        # Determine emoji
        pnl_emoji = "🟢" if gross_pnl >= 0 else "🔴"
        
        position_msg = f"""📊 **POSICIÓN: {symbol}**
═══════════════════════════════

🤖 **Worker:** {worker_name}
📅 **Entrada:** {entry_time_str[:19] if len(entry_time_str) >= 19 else entry_time_str}
💰 **Precio entrada:** ${entry_price:.2f}
📈 **Precio actual:** ${current_price:.2f}
📊 **Cantidad:** {quantity:,} shares
{"• Valor posición: $" + f"{position_value:,.2f}" if position_value else ""}

💵 **P&L:**
• Bruto: {pnl_emoji} ${gross_pnl:+.2f} ({pnl_pct:+.2f}%)

🎯 **Targets:**
• Take Profit: ${tp_price:.2f} (+{take_profit_pct:.2f}%)
• Stop Loss: ${sl_price:.2f} (-{stop_loss_pct:.2f}%)
...
⏰ {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"""
        ...
        send_message(position_msg, use_html=True)
        _logger.info(f"Sent position details for {symbol}")
        
    except Exception as e:
        _logger.error(f"Error getting position details for {symbol}: {e}")
        send_message(f"❌ Error obteniendo detalles de {symbol}: {str(e)}")

def send_summary():
    """Send executive summary of the day"""
    if not is_enabled():
        return
    ...
    try:
        ...
        if db_path.exists():
            try:
                with sqlite3.connect(db_path) as conn:
                    ...
                    for symbol, pnl, strategy in trades:
                        if pnl:
                            total_pnl += pnl
                            if pnl > 0:
                                winning_trades += 1
                                if not best_trade or pnl > best_trade[1]:
                                    best_trade = (symbol, pnl, strategy)
                            else:
                                losing_trades += 1
                                if not worst_trade or pnl < worst_trade[1]:
                                    worst_trade = (symbol, pnl, strategy)
            except Exception as db_error:
                _logger.warning(f"Could not get DB stats: {db_error}")
        
        # Get current positions from workers
        open_positions = 0
        active_workers = 0
        worker_positions = {}
        
        unified_system = service_locator.get_service('unified_trading_system')
        if unified_system and hasattr(unified_system, 'strategy_engine'):
            strategy_engine = unified_system.strategy_engine
            if hasattr(strategy_engine, 'workers'):
                for worker_name, worker in strategy_engine.workers.items():
                    if hasattr(worker, 'active_positions'):
                        pos_count = len(worker.active_positions)
                        if pos_count > 0:
                            open_positions += pos_count
                            active_workers += 1
                            worker_positions[worker_name] = pos_count
        
        # Calculate win rate
        win_rate = (winning_trades / trades_today * 100) if trades_today > 0 else 0
        
        # Format best/worst trades
        best_trade_str = f"{best_trade[0]}: +${best_trade[1]:.2f}" if best_trade else "N/A"
        worst_trade_str = f"{worst_trade[0]}: ${worst_trade[1]:.2f}" if worst_trade else "N/A"
        
        # Format top workers
        top_workers_str = ""
        if worker_positions:
            sorted_workers = sorted(worker_positions.items(), key=lambda x: x[1], reverse=True)[:3]
            for i, (wname, count) in enumerate(sorted_workers, 1):
                top_workers_str += f"\n{i}. {wname}: {count} pos"
        else:
            top_workers_str = "\nNo active positions"
        ...
        summary_msg = f"""📊 **RESUMEN EJECUTIVO** - {today}
═══════════════════════════════

💰 **RENDIMIENTO:**
• P&L Neto: {status_emoji} ${total_pnl:+.2f}
• Win Rate: {win_rate:.1f}% ({winning_trades}W / {losing_trades}L)
• Mejor trade: {best_trade_str}
• Peor trade: {worst_trade_str}

📈 **ACTIVIDAD:**
• Trades ejecutados: {trades_today}
• Posiciones abiertas: {open_positions}
• Workers activos: {active_workers}

🎯 **TOP WORKERS:**{top_workers_str}

═══════════════════════════════
⏰ {datetime.now().strftime('%H:%M:%S')}"""
        
        send_message(summary_msg, use_html=True)
        _logger.info(f"Sent executive summary: {trades_today} trades, ${total_pnl:.2f} PnL")
        
    except Exception as e:
        _logger.error(f"Error generating summary: {e}")
        send_message(f"❌ Error generando resumen: {str(e)}")
...
def handle_command(text: str):
    ...
    if text.startswith("/start") or text.startswith("/menu"):
        ...
        if len(parts) >= 2:
            ...
            send_position_details(symbol)
        else:
            send_message("❓ Uso: /position <SYMBOL>\\n\\nEjemplo: /position AAPL")
    elif text == "/summary":
        send_summary()
    elif text == "/help":
        send_help_message()
