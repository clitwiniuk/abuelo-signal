...
class ExtendedHoursManager:
    ...
    def get_market_session(self) -> MarketSession:
        ...
        try:
            ...
            current_time_obj = self.clock.now()
            
            # Convert to US Eastern time
            # If timestamp is naive (SimulatedTimeProvider often returns naive), assume it's already market time (ET) or handles its own timezone
            # For safe handling:
            if current_time_obj.tzinfo is None:
                # Assume naive timestamps in simulation are usually Market Time (ET)
                # But SystemTimeProvider returns naive local time.
                # To distinguish, check provider type or assume config. 
                # For now, let's assume naive = system local time if SystemTimeProvider, and ET if Simulated.
                # Actually, SystemTimeProvider().now() is naive local.
                # datetime.now(ZoneInfo("America/New_York")) handles naive local to aware ET conversion if using system time.
                
                # If we are using SystemTimeProvider, we want real conversion.
                if isinstance(self.clock, SystemTimeProvider):
                     us_now = current_time_obj.astimezone(ZoneInfo("America/New_York"))
                else:
                     # Simulated time - Assume it is already compatible or ET
                     # Most historical data in this system seems to be stored as naive datetime but implicitly ET/Market Time
                     us_now = current_time_obj.replace(tzinfo=ZoneInfo("America/New_York"))
            else:
                us_now = current_time_obj.astimezone(ZoneInfo("America/New_York"))
    ...
    def get_session_info(self) -> dict:
        ...
        # Get US Eastern time for display
        current_time_obj = self.clock.now()
        if current_time_obj.tzinfo is None:
            if isinstance(self.clock, SystemTimeProvider):
                 us_now = current_time_obj.astimezone(ZoneInfo("America/New_York"))
