...
class DatabaseManager:
    ...
    def _create_tables(self, conn: sqlite3.Connection):
        ...
        # Scanner signals table - Persist scanner opportunities for replay
        conn.execute("""
            CREATE TABLE IF NOT EXISTS scanner_signals (
                ...
                full_signal_json TEXT, -- JSON serialization of entire opportunity
                created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
            )
        """)
        
        # Create indexes for scanner_signals
        conn.execute("CREATE INDEX IF NOT EXISTS idx_scanner_symbol ON scanner_signals(symbol)")
        conn.execute("CREATE INDEX IF NOT EXISTS idx_scanner_date ON scanner_signals(signal_date)")

        # Swing trades table - Position trades held days/weeks/months
        conn.execute("""
        ...
        # Swing picks cache - Avoid re-picking same symbol too soon
        conn.execute("""
        ...
        # Proactive Candidates table - For Proactive Scanner (Days 0-7 tracking)
        conn.execute("""
        ...
        # Create indexes for proactive candidates
        conn.execute("CREATE INDEX IF NOT EXISTS idx_proactive_symbol ON proactive_candidates(symbol)")
        conn.execute("CREATE INDEX IF NOT EXISTS idx_proactive_date ON proactive_candidates(detection_date)")
        conn.execute("CREATE INDEX IF NOT EXISTS idx_proactive_status ON proactive_candidates(status)")
        ...
        # Swing trades indexes
        conn.execute("CREATE INDEX IF NOT EXISTS idx_swing_symbol ON swing_trades(symbol)")
        conn.execute("CREATE INDEX IF NOT EXISTS idx_swing_status ON swing_trades(status)")
        conn.execute("CREATE INDEX IF NOT EXISTS idx_swing_scan_date ON swing_trades(scan_date)")
        conn.execute("CREATE INDEX IF NOT EXISTS idx_swing_entry_date ON swing_trades(entry_date)")

        self.logger.info("Database tables created successfully (day + swing trading)")
    ...
    def save_trade(self, trade_data: Dict[str, Any]) -> bool:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                if existing:
                    # DYNAMIC UPDATE: Only update fields present in trade_data
                    # This prevents overwriting existing data with NULLs during partial updates (e.g. exits)
                    
                    # 1. Identify valid columns in the trades table
                    # (We could cache this schema, but for safety we'll rely on known columns or just try/except)
                    # For now, we'll trust that trade_data keys map to columns, or we filter them.
                    # Let's filter against a known list of columns to be safe, or just iterate.
                    # Given the previous code had a hardcoded list, let's use a dynamic approach but careful about keys.
                    
                    # List of all possible columns in the trades table (based on schema)
                    valid_columns = {
                        'symbol', 'strategy', 'side', 'quantity', 'entry_price', 'exit_price',
                        'entry_time', 'exit_time', 'duration_minutes', 'pnl', 'commission', 'status',
                        ...
                        'trade_session', 'market_context', 'signal_strength', 'market_context_score',
                        'strategy_confidence', 'volume_ratio', 'gap_percentage',
                        'expected_value_pct', 'risk_reward_ratio', 'win_probability',
                        'trade_tier', 'adaptive_risk_pct', 'broker_order_id_entry',
                        'trading_horizon', 'expected_hold_hours', 'EOD_safe', 'worker_name',
                        'daily_rsi', 'distance_to_resistance_pct', 'resistance_price',
                        'ods_classification', 'ods_strength', 'invalid_price', 
                        'suggested_sl_price', 'suggested_tp_price',
                        'stop_loss_pct', 'take_profit_pct', 'trailing_activation_pct', 'trailing_distance_pct', 'stop_loss_price', 'take_profit_price'
                    }
                    ...
                    for key, value in trade_data.items():
                        if key in valid_columns and key != 'trade_id': # trade_id is in WHERE clause
                            set_clauses.append(f"{key} = ?")
                            values.append(value)
                    
                    if not set_clauses:
                        self.logger.warning(f"No valid columns to update for trade {trade_data['trade_id']}")
                        return True # Nothing to update
                        
                    set_clauses.append("updated_at = CURRENT_TIMESTAMP")
                    values.append(trade_data['trade_id']) # For WHERE clause
                    ...
                    conn.execute(f"INSERT INTO trades ({columns_str}) VALUES ({placeholders})", (
                        ...
                        trade_data.get('trade_session'),
                        trade_data.get('market_context'),
                        trade_data.get('signal_strength'),
                        trade_data.get('market_context_score'),
                        trade_data.get('strategy_confidence'),
                        trade_data.get('volume_ratio'),
                        trade_data.get('gap_percentage'),
                        trade_data.get('expected_value_pct'),
                        trade_data.get('risk_reward_ratio'),
                        trade_data.get('win_probability'),
                        trade_data.get('trade_tier'),
                        trade_data.get('adaptive_risk_pct'),
                        trade_data.get('broker_order_id_entry'),
                        trade_data.get('trading_horizon', 'INTRADAY'),
                        trade_data.get('expected_hold_hours', 6.0),
                        trade_data.get('EOD_safe', 0),
                        trade_data.get('worker_name'),
                        trade_data.get('daily_rsi'),
                        trade_data.get('distance_to_resistance_pct'),
                        trade_data.get('resistance_price'),
                        trade_data.get('ods_classification'),
                        trade_data.get('ods_strength'),
                        trade_data.get('invalid_price'),
                        trade_data.get('suggested_sl_price'),
                        trade_data.get('suggested_tp_price'),
                        trade_data.get('stop_loss_pct'),
                        trade_data.get('take_profit_pct'),
                        trade_data.get('trailing_activation_pct'),
                        trade_data.get('trailing_distance_pct'),
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                for row in rows:
                    trades.append({
                        ...
                        # Exit parameters for restoration
                        ...
                        'trailing_distance_pct': row['trailing_distance_pct'] if 'trailing_distance_pct' in row.keys() else None
                    })
    ...
    def close_trade(self, trade_id: str, exit_price: float, exit_reason: str,
                    pnl: float = 0.0, commission: float = 0.0) -> bool:
        """
        ...
        try:
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                conn.commit()
                self.logger.debug(f"✅ Closed trade {trade_id}: {exit_reason}")
                return True
