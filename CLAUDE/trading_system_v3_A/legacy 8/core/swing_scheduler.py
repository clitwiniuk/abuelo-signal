...
class SwingScheduler:
    ...
    def __init__(self, scanner, worker, logger: Optional[logging.Logger] = None):
        ...
        Args:
            ...
            worker: Primary worker (ConsolidationBreakoutWorker)
            logger: Optional logger
        """
        self.scanner = scanner
        self.workers = [worker]  # List of registered workers
        ...
        self.market_close = time(20, 0)         # Monitor until 20:00 ET (Aftermarket Close)
    ...
    def register_worker(self, worker):
        ...
        if worker not in self.workers:
            ...
            self.logger.info(f"✅ Registered additional worker: {worker.worker_name}")
        ...
        while self.running:
            try:
                ...
                if should_run:
                    ...
                    # Safety check: Ensure scanner is initialized
                    if self.scanner is None:
                        self.logger.warning(
                            "⚠️ EOD scan skipped - scanner not initialized. "
                            "Swing trading may be disabled or scanner failed to initialize."
                        )
            ...
            try:
                ...
                if should_check and len(self.swing_picks) > 0:
                    ...
                    for pick in self.swing_picks:
                        ...
                        # Try to enter using all registered workers
                        self.logger.info(f"🎯 {symbol}: Attempting BREAKOUT entry...")
                        
                        success = False
                        for worker in self.workers:
                            # Check if this worker handles this type of pick
                            # (For now, just try all, they will reject if criteria not met)
                            if await worker.process_swing_pick(pick):
                                success = True
                                self.logger.info(f"✅ {symbol}: Entry executed by {worker.worker_name}")
                                break # Stop after first successful entry
                ...
                if is_market_hours and len(self.swing_picks) > 0:
                    ...
                    for pick in self.swing_picks:
                        ...
                        for worker in self.workers:
                            if await worker.process_swing_pick(pick):
                                success = True
                                self.logger.info(f"✅ {symbol}: PULLBACK entry executed by {worker.worker_name}")
                                break
            ...
            try:
                ...
                if is_market_hours and len(self.active_positions) > 0:
                    ...
                    for symbol, position in self.active_positions.items():
                        ...
                        # Check exit conditions using the worker that owns the strategy?
                        # Or just try all workers until one handles it
                        exited = False
                        for worker in self.workers:
                            # Ideally we check if position['strategy'] == worker.worker_name
                            # But for now, let's just try
                            if await worker.check_position_exit(symbol, position):
                                exited = True
                                break
