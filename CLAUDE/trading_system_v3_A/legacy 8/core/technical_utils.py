...
class TechnicalUtils:
    ...
    def calculate_roc(prices: List[float], period: int) -> Optional[float]:
        """
        Rate of Change - Cambio porcentual sobre N períodos

        Args:
            prices: Lista de precios
            period: Número de períodos para calcular ROC

        Returns:
            ROC como decimal (0.15 = 15% ganancia) o None si no hay suficientes datos
        """
        if len(prices) < period + 1:
            return None
    ...
    @staticmethod
    def calculate_acceleration(prices: List[float], short_period: int = 3,
                              long_period: int = 10) -> Optional[float]:
        """
        Acceleration Score - Mide si el momentum está acelerando

        Compara ROC de corto plazo vs largo plazo.
        Valor > 1.0 indica aceleración (momentum creciente)

        Args:
            prices: Lista de precios
            short_period: Período corto para ROC
            long_period: Período largo para ROC

        Returns:
            Ratio de aceleración o None si no hay suficientes datos
        """
        if len(prices) < long_period + 1:
            return None

        roc_short = TechnicalUtils.calculate_roc(prices, short_period)
        roc_long = TechnicalUtils.calculate_roc(prices, long_period)

        if roc_short is None or roc_long is None or roc_long == 0:
            return None

        # Si ambos son positivos, calcular ratio
        if roc_short > 0 and roc_long > 0:
            acceleration_ratio = roc_short / roc_long
