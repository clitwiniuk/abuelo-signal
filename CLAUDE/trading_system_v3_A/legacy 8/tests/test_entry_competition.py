...
class TestEntryCompetition(unittest.IsolatedAsyncioTestCase):
    async def test_competition_winner(self):
        ...
        async def worker_b():
            return await comp.register_entry(symbol, "WorkerB", 90.0)
            
        async def worker_c():
            return await comp.register_entry(symbol, "WorkerC", 85.0)
    ...
    async def test_competition_tie_breaker(self):
        ...
        async def worker_x():
            return await comp.register_entry(symbol, "WorkerX", 90.0)
            
        async def worker_y():
            return await comp.register_entry(symbol, "WorkerY", 90.0)
