...
class TradingExecutionStage(PipelineStage):
        ...
        try:
            ...
            for symbol in closed_symbols:
                if symbol in self.active_trades:
                    ...
                    # Check for rejected/cancelled entry order
                    # This handles cases where order was placed but rejected by broker (e.g. permission issues)
                    broker_order_id = active_trade.get('broker_order_id_entry')
                    if broker_order_id:
                        try:
                            order_status = await self.broker.get_order_status(broker_order_id)
                            if order_status:
                                status = order_status.get('status')
                                if status in ['REJECTED', 'CANCELLED', 'INACTIVE']:
                                    self.logger.warning(f"🚫 Entry order {status} for {symbol} (ID: {broker_order_id}) - cleaning up")
                                    await self._handle_failed_entry(symbol, active_trade, status)
                                    continue
                        except Exception as e:
    ...
    async def _handle_failed_entry(self, symbol: str, active_trade: Dict[str, Any], reason: str):
        """Handle failed entry order (Rejected/Cancelled) - clean up database"""
        try:
            self.logger.info(f"🧹 Cleaning up failed entry for {symbol} (Reason: {reason})")
            
            # Update trade record in database
            trade_data = {
                'trade_id': active_trade['trade_id'],
                'status': 'REJECTED' if reason == 'REJECTED' else 'CANCELLED',
                'notes': f"{active_trade.get('notes', '')} | Entry failed: {reason}".strip(' |'),
            ...
            # We use save_trade which updates based on trade_id
            success = self.db_manager.save_trade(trade_data)
            
            if success:
                self.logger.info(f"💾 Failed trade updated in DB: {active_trade['trade_id']} -> {trade_data['status']}")
            else:
                self.logger.error(f"❌ Failed to update failed trade in DB for {symbol}")
            ...
            self.logger.error(f"❌ Error handling failed entry for {symbol}: {e}")
    ...
    async def _handle_entry_trade(self, signal: Signal, order: Order, order_id: str, trade_id: str, strategy_name: str):
        ...
        try:
            ...
            market_context_score = self._calculate_market_context_score(signal, strategy_name)

            # Determine trade session
            trade_session = self._determine_trade_session()
            ...
            trade_data = {
                ...
                'market_context_score': market_context_score,  # Add market context score
                'trade_session': trade_session,  # Add trade session
                'notes': f"Entry signal: {signal.signal_type.value}, Confidence: {confidence}%, Context: {market_context_score}%, Session: {trade_session}, Order ID: {order_id}, Commission: ${entry_commission:.2f}",
                'broker_order_id_entry': str(order_id),  # CRITICAL: Save broker order ID for execution tracking
            ...
            if success:
                ...
                self.active_trades[signal.symbol] = {
                    ...
                    'broker_order_id_entry': str(order_id)  # CRITICAL: Store broker order ID for status checks
                }
        ...
        try:
            ...
            if signal.symbol in self.active_trades:
                ...
                trade_data = {
                    ...
                    'broker_order_id_exit': str(order_id)  # CRITICAL: Save broker order ID for execution tracking
                }
    ...
    def _calculate_market_context_score(self, signal: Signal, strategy_name: str) -> float:
        """
        Calculate market context score (0-100) based on market conditions
        ...
        Returns:
            float: Market context score (0-100)
        """
        try:
            base_score = 60.0  # Neutral base

            # Check signal metadata for market context
            if hasattr(signal, 'metadata') and signal.metadata:
                # Look for market regime or trend info
                ...
                # Check for SPY trend
                spy_trend = signal.metadata.get('spy_trend')
            ...
            if 'Gap' in strategy_name or 'Breakout' in strategy_name or 'gap_go' in strategy_name:
                base_score += 10  # These work better in trending markets
            ...
            self.logger.debug(f"Error calculating market context score: {e}")
            return 60.0  # Default neutral score

    def _determine_trade_session(self, entry_time: datetime = None) -> str:
        """
        Determine trading session based on entry time

        Args:
            entry_time: Trade entry timestamp (defaults to now)

        Returns:
            str: Session name (premarket, first_hour, midday, power_hour, afterhours)
        """
        try:
            ...
            if time_decimal < 9.5:  # Before 9:30 AM
                return 'premarket'
            elif 9.5 <= time_decimal < 10.5:  # 9:30-10:30 AM
                return 'first_hour'
            elif 10.5 <= time_decimal < 15.0:  # 10:30 AM - 3:00 PM
                return 'midday'
            elif 15.0 <= time_decimal < 16.0:  # 3:00-4:00 PM
                return 'power_hour'
            else:  # After 4:00 PM
                return 'afterhours'

        except Exception as e:
            self.logger.debug(f"Error determining trade session: {e}")
