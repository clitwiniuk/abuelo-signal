...
class TradeOHLCRecorder:
    ...
    def record_market_data(self, symbol: str, bar: MarketData):
        ...
        with self._lock:
            ...
            current_bars = self._daily_data_cache[symbol][trading_date]
            
            # Check for duplicates or updates
            # Optimization: Check last bar first (most common case in streaming)
            if current_bars and current_bars[-1].timestamp == bar.timestamp:
                current_bars[-1] = bar
                return

            # Check if we need to insert or update elsewhere (backfilling case)
            # If bar is newer than last one, just append
            if not current_bars or bar.timestamp > current_bars[-1].timestamp:
                current_bars.append(bar)
            else:
                # Linear search for update or insertion point
                # This handles out-of-order arrival or backfilling
                updated = False
                for i, existing_bar in enumerate(current_bars):
                    if existing_bar.timestamp == bar.timestamp:
                        current_bars[i] = bar
                        updated = True
                        break
                    elif existing_bar.timestamp > bar.timestamp:
                        current_bars.insert(i, bar)
                        updated = True
                        break
    ...
    def start_trade_recording(self, signal: Signal) -> bool:
        ...
        Soporta REANUDACIÓN: Si ya existe un snapshot para este símbolo/día en la DB,
        lo carga y continúa añadiendo datos en lugar de crear uno nuevo.
        """
        try:
            with self._lock:
                ...
                # REANUDACIÓN: Verificar si ya existe snapshot en DB para evitar duplicados
                existing_snapshot = self.get_existing_snapshot(symbol, trading_date)
                
                if existing_snapshot:
                    self.logger.info(f"🔄 Resuming existing OHLC recording for {symbol} (Trade ID: {existing_snapshot.trade_id})")
                    trade_id = existing_snapshot.trade_id
                    
                    # Cargar barras previas en memoria si no están
                    if existing_snapshot.intraday_bars:
                        try:
                            prev_bars_dicts = json.loads(existing_snapshot.intraday_bars)
                            # Convertir dicts a MarketData para el cache
                            # Nota: Esto es simplificado, idealmente tendríamos un método de conversión inverso
                            # pero para el propósito de append funciona
                            pass 
                        except:
                            self.logger.warning(f"⚠️ Could not parse existing intraday bars for resuming {symbol}")
                    
                    # Usar el snapshot existente
                    ...
                    if not daily_bars:
                        self.logger.warning(f"⚠️ No daily bars available for {symbol} on {trading_date} (Starting empty)")
                        # Permitimos arrancar vacío para no perder el trade, se llenará con updates
                        day_open = signal.price
                        day_high = signal.price
                        day_low = signal.price
                        day_volume = 0
                    else:
                    ...
                    entry_bar = self._find_closest_bar(daily_bars, signal.timestamp) if daily_bars else None
                    ...
                    if signal.timestamp.hour == 9 and signal.timestamp.minute <= 35:
                        market_open_price = day_open
                        if len(daily_bars) > 1:
                            prev_close = daily_bars[0].open
                    ...
                    snapshot = TradeOHLCSnapshot(
                        ...
                        intraday_bars=json.dumps([self._bar_to_dict(bar) for bar in daily_bars]) if daily_bars else "[]"
                    )

                # Almacenar en cache de trades activos
                self._active_trades[trade_id] = snapshot

                # SAFETY SAVE: Guardar inmediatamente en DB para prevenir pérdida de datos
                self._save_snapshot_to_db(snapshot)

                self.logger.info(f"📊 Started/Resumed OHLC recording for trade {trade_id[:8]}... ({symbol})")
                return True
    ...
    def get_existing_snapshot(self, symbol: str, trading_date: str) -> Optional[TradeOHLCSnapshot]:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                # Manejo robusto de fechas (pueden ser string iso o objetos datetime si viene de adaptador)
                entry_time = row['entry_time']
                if isinstance(entry_time, str):
                    try:
                        entry_time = parser.parse(entry_time) if 'parser' in globals() else datetime.fromisoformat(entry_time)
                    except:
                        pass # Keep as string if parsing fails or let it crash cleanly later

                exit_time = row['exit_time']
                if exit_time and isinstance(exit_time, str):
                    try:
                        exit_time = parser.parse(exit_time) if 'parser' in globals() else datetime.fromisoformat(exit_time)
                    except:
                        pass
