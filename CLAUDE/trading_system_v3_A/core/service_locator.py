...
class UnifiedConfig:
    ...
    paper_trading_mode: bool = True  # Default to paper trading for safety
    ...
    buy_the_dip_worker_enabled: bool = True  # Buy the dip in uptrends (dip buying strategy)
    buy_and_hold_worker_enabled: bool = True  # Buy and hold immediate entry on scanner opportunities
    ...
    daily_plays_strategy_enabled: bool = True  # Daily Plays worker (smallcap)
    daily_plays_midcap_strategy_enabled: bool = True  # Daily Plays Mid-Cap worker
    ...
    vcp_strict_strategy_enabled: bool = False  # VCP Strict worker (MANDATORY VWAP slope validation)
    balance_day_strategy_enabled: bool = True  # Balance Day Range Trading worker
    livermore_intraday_strategy_enabled: bool = True  # Livermore Intraday worker (replaces Momentum Breakout)
    holy_grail_strategy_enabled: bool = True  # The Holy Grail ADX worker
    ...
    # Simple Robust Workers (minimal filters, high robustness)
    smallcap_momentum_scalper_enabled: bool = True  # Simple momentum scalper for smallcaps
    
    # Parabolic Extension Worker (EARLY-stage parabolic acceleration)
    parabolic_strategy_enabled: bool = True  # Parabolic Extension worker
    
    # Short Parabolic Reversal Worker (LATE-stage parabolic exhaustion)
    short_parabolic_strategy_enabled: bool = True  # Short Parabolic worker

    # Gap Fade Worker (SHORT strategy - gaps without catalyst)
    gap_fade_strategy_enabled: bool = True  # Gap Fade worker (62-68% win rate expected)

    # Short Squeeze Worker (Proactive Scanner + VWAP Reclaim execution)
    short_squeeze_strategy_enabled: bool = True  # Short Squeeze worker (Day 0-7 proactive candidates)

    # Catalyst DNA Worker (Market DNA adapted for smallcaps - LONG only)
    catalyst_dna_strategy_enabled: bool = True  # Catalyst DNA worker (70-75% win rate target)

    # Small Caps Short Reversal Worker (Mean Reversion SHORT strategy)
    smallcaps_short_reversal_enabled: bool = True  # Small Caps Short Reversal worker (60-70% win rate expected)

    # Smallcap VWAP Runner Worker (Simple robust strategy)
    smallcap_vwap_runner_enabled: bool = True
    ...
    def load_config(self, config_file: str = "config.ini") -> UnifiedConfig:
        ...
        try:
            ...
            self._config = UnifiedConfig(
                ...
                max_daily_loss=parser.getfloat('TRADING', 'max_daily_loss', fallback=
                    -1 * abs(parser.getfloat('TRADING', 'portfolio_capital', fallback=2000.0) * 
                             (parser.getfloat('TRADING', 'max_daily_loss_pct', fallback=5.0) / 100.0))
                ),
                ...
                paper_trading_mode=parser.getboolean('TRADING', 'paper_trading_mode', fallback=True),
                ...
                buy_the_dip_worker_enabled=parser.getboolean('BUY_THE_DIP_WORKER', 'enabled', fallback=True),
                buy_and_hold_worker_enabled=parser.getboolean('BUY_AND_HOLD_WORKER', 'enabled', fallback=True),
                ...
                daily_plays_midcap_strategy_enabled=parser.getboolean('DAILY_PLAYS_MIDCAP_STRATEGY', 'enabled', fallback=True),
                ...
                vcp_strict_strategy_enabled=parser.getboolean('VCP_STRICT_STRATEGY', 'enabled', fallback=False),
                livermore_intraday_strategy_enabled=parser.getboolean('LIVERMORE_INTRADAY_STRATEGY', 'enabled', fallback=True),
                holy_grail_strategy_enabled=parser.getboolean('HOLY_GRAIL_STRATEGY', 'enabled', fallback=True),
                ...
                # Simple Robust Workers
                smallcap_momentum_scalper_enabled=parser.getboolean('SMALLCAP_MOMENTUM_SCALPER', 'enabled', fallback=True),
                
                # Parabolic Extension Worker
                parabolic_strategy_enabled=parser.getboolean('PARABOLIC_STRATEGY', 'enabled', fallback=True),

                # Short Parabolic Worker
                short_parabolic_strategy_enabled=parser.getboolean('SHORT_PARABOLIC_STRATEGY', 'enabled', fallback=True),

                # Gap Fade Worker
                gap_fade_strategy_enabled=parser.getboolean('GAP_FADE_STRATEGY', 'enabled', fallback=True),

                # Short Squeeze Worker
                short_squeeze_strategy_enabled=parser.getboolean('SHORT_SQUEEZE_STRATEGY', 'enabled', fallback=True),

                # Catalyst DNA Worker
                catalyst_dna_strategy_enabled=parser.getboolean('CATALYST_DNA_WORKER', 'enabled', fallback=True),
                ...
                # Smallcap VWAP Runner Worker
                smallcap_vwap_runner_enabled=parser.getboolean('SMALLCAP_VWAP_RUNNER', 'enabled', fallback=True)
            )
            ...
            # Collect all workers and their status
            all_workers = {
                'volume_absorption': self._config.volume_absorption_worker_enabled,
                'buy_the_dip': self._config.buy_the_dip_worker_enabled,
                'buy_and_hold': self._config.buy_and_hold_worker_enabled,
                'generic_01': self._config.generic_01_strategy_enabled,
                'smallcaps_long': self._config.smallcaps_long_strategy_enabled,
                'outlier_penny_extreme': self._config.outlier_penny_extreme_strategy_enabled,
                'macdv': self._config.macdv_strategy_enabled,
                'daily_plays': self._config.daily_plays_strategy_enabled,
                'daily_plays_midcap': self._config.daily_plays_midcap_strategy_enabled,
                'orb': self._config.orb_strategy_enabled,
                'balance_day': self._config.balance_day_strategy_enabled,
                'vwap_breakout': self._config.vwap_breakout_strategy_enabled,
                'momentum_breakout': self._config.momentum_breakout_strategy_enabled,
                'vcp': self._config.vcp_strategy_enabled,
                'vcp_strict': self._config.vcp_strict_strategy_enabled,
                'livermore_intraday': self._config.livermore_intraday_strategy_enabled,
                'holy_grail': self._config.holy_grail_strategy_enabled,
                'ods_universal': self._config.ods_universal_strategy_enabled,
                'ods_swing_universal': self._config.ods_swing_universal_strategy_enabled,
                'smallcap_momentum_scalper': self._config.smallcap_momentum_scalper_enabled,
                'parabolic': self._config.parabolic_strategy_enabled,
                'short_parabolic': self._config.short_parabolic_strategy_enabled,
                'gap_fade': self._config.gap_fade_strategy_enabled,
                'short_squeeze': self._config.short_squeeze_strategy_enabled,
                'smallcaps_short_reversal': self._config.smallcaps_short_reversal_enabled,
                'smallcap_vwap_runner': self._config.smallcap_vwap_runner_enabled,
            }

            # Show only enabled workers
            enabled_workers = [name for name, enabled in all_workers.items() if enabled]
            disabled_workers = [name for name, enabled in all_workers.items() if not enabled]

            if enabled_workers:
                self._logger.info(f"     ✅ Active Workers ({len(enabled_workers)}):")
                for worker in enabled_workers:
                    self._logger.info(f"        - {worker}")

            if disabled_workers:
                self._logger.info(f"     ❌ Disabled Workers ({len(disabled_workers)}):")
    ...
    async def get_or_create_ibkr_adapter(self):
        ...
        if adapter is None:
            ...
            if config.production_mode:
                ...
                adapter = IBKRAdapter(client_id=config.client_id, config=config)
            else:
                from adapters.mock_ibkr_adapter import MockIBKRAdapter
                adapter = MockIBKRAdapter(client_id=config.client_id, config=config)
