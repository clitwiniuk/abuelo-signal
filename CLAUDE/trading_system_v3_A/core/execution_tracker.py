...
class ExecutionTracker:
    ...
    def __init__(self, db_path: str = "trading_data.db"):
        ...
        # Pending fills buffer: {symbol: [fill_data, ...]}
    ...
    def record_execution(self, 
                        ...
                        buffer_if_missing: bool = True) -> bool:
        """
            ...
            try:
                with sqlite3.connect(self.db_path) as conn:
                    ...
                    if trade_id:
                        ...
                        # Attempt 1: Direct lookup by broker_order_id
                        cursor.execute("""
                            SELECT * FROM trades
                        ...
                        # Attempt 2: Fallback lookup by symbol (if ID not found/matched)
                        if not trade:
                            cursor.execute("""
                                SELECT * FROM trades
                    ...
                    if not trade:
                        if buffer_if_missing:
                            # Trade STILL not found - buffer this fill
                            self.logger.info(f"⏳ Trade not found for {symbol} - Buffering fill for later matching (ID: {broker_order_id})")
    ...
    def process_pending_fills(self, symbol: str):
        """
        Process any buffered fills for a symbol.
        Call this immediately after creating a new trade to match "fast fills".
        """
        with self._lock:
            if symbol not in self._pending_fills or not self._pending_fills[symbol]:
                return

            fills = self._pending_fills[symbol]
            # Use a copy to iterate because we might modify the list (or at least we want to rebuild it)
            # Actually, we will retry recording each fill. If it succeeds, we remove it.
            
            remaining_fills = []
            matched_count = 0
            
            self.logger.info(f"🔄 Processing {len(fills)} pending fills for {symbol}...")
            
            for fill in fills:
                # Call record_execution with buffer_if_missing=False
                # Since we use RLock, it is safe to call recursively
                
                success = self.record_execution(
                    symbol=fill['symbol'],
                    side=fill['side'],
            ...
            self._pending_fills[symbol] = remaining_fills
            
            if matched_count > 0:
                self.logger.info(f"✅ Successfully matched {matched_count} pending fills for {symbol}")
