...
class CatalystAnalyzer:
    ...
    def __init__(self, intraday_config: Optional[Dict[str, Any]] = None, config: Optional[Dict[str, Any]] = None):
        self.logger = logging.getLogger(f"{__name__}.CatalystAnalyzer")
        
        # Store full configuration for feature flags
    ...
    def analyze_headline(self, headline: str, news_age_hours: float = 0.0,
                        gap_pct: float = 0.0, volume_ratio: float = 0.0) -> CatalystInfo:
        """
        ...
        Args:
            ...
            gap_pct: Gap percentage (for sentiment validation)
            volume_ratio: Volume ratio (for sentiment validation)
        ...
        if self.finbert_analyzer:
            try:
                ...
                # PHASE 3: Validate sentiment (optional, feature-flag controlled)
                sentiment_valid = self._validate_sentiment(
                    finbert_result, gap_pct, volume_ratio, headline
                )

                if not sentiment_valid:
                    # Reject negative sentiment event
                    self.logger.info(f"❌ REJECTED negative sentiment: '{headline[:60]}...'")
                    return CatalystInfo(
                        catalyst_type='OTHER',
                        strength=1,  # Minimal strength
    ...
    def analyze_multiple_headlines(self, headlines: List[Tuple[str, float]],
                                   gap_pct: float = 0.0, volume_ratio: float = 0.0) -> CatalystInfo:
        """
        ...
        for headline, age_hours in headlines:
            catalyst = self.analyze_headline(headline, age_hours, gap_pct, volume_ratio)
    ...
    def _validate_sentiment(self, finbert_result, gap_pct: float, volume_ratio: float, headline: str) -> bool:
        """
        PHASE 3: Validate FinBERT sentiment to filter negative events

        Rejection logic for smallcaps:
        - Negative sentiment + weak gap (<15%) + weak volume (<3x) → REJECT (trap)
        - Negative sentiment + massive gap (≥15%) + strong volume (≥3x) → ACCEPT (short squeeze)
        - Neutral sentiment → ACCEPT (FDA, M&A often neutral)
        - Positive sentiment → ACCEPT

        Args:
            finbert_result: FinBERTResult with sentiment and confidence
            gap_pct: Gap percentage from previous close
            volume_ratio: Volume ratio vs average
            headline: News headline (for logging)

        Returns:
            bool: True if sentiment is acceptable, False to reject
        """
        # Feature flag check
        event_driven_config = self.config.get('event_driven_config', {})
        if not event_driven_config.get('enable_finbert_filter', False):
            # Filter disabled → accept all (backward compatible)
            return True

        sentiment = finbert_result.sentiment.lower()
        confidence = finbert_result.confidence

        # Config parameters
        negative_threshold = event_driven_config.get('finbert_negative_threshold', -0.5)
        accept_neutral = event_driven_config.get('accept_neutral_sentiment', True)

        # Map sentiment to numeric score for comparison
        ...
        # CASE 1: Positive sentiment → always accept
        if sentiment == 'positive':
            self.logger.debug(
                f"✅ Sentiment ACCEPTED: POSITIVE ({confidence:.2f}) - '{headline[:50]}...'"
            )
            return True

        # CASE 2: Neutral sentiment → accept if configured (default: yes)
        if sentiment == 'neutral':
            if accept_neutral:
                self.logger.debug(
                    f"✅ Sentiment ACCEPTED: NEUTRAL ({confidence:.2f}) - '{headline[:50]}...'"
                )
                return True
            else:
                self.logger.info(
                    f"❌ Sentiment REJECTED: NEUTRAL not allowed - '{headline[:50]}...'"
                )
                return False

        # CASE 3: Negative sentiment → validate against price action
        if sentiment == 'negative':
            # Short squeeze detection: massive gap + strong volume override negative sentiment
            is_short_squeeze = (gap_pct >= 15.0 and volume_ratio >= 3.0)

            if is_short_squeeze:
                self.logger.info(
                    f"✅ Sentiment ACCEPTED despite NEGATIVE: Short squeeze detected "
                    f"(gap={gap_pct:.1f}%, vol={volume_ratio:.1f}x) - '{headline[:50]}...'"
                )
                return True

            # Negative sentiment + weak price action → REJECT (trap)
            self.logger.info(
                f"❌ Sentiment REJECTED: NEGATIVE ({confidence:.2f}) without short squeeze "
                f"(gap={gap_pct:.1f}%, vol={volume_ratio:.1f}x) - '{headline[:50]}...'"
            )
            return False

        # Fallback: accept unknown sentiment (fail-safe)
        self.logger.warning(
            f"⚠️ Unknown sentiment '{sentiment}' - defaulting to ACCEPT (fail-safe)"
        )
        return True