...
class ParabolicWorkerLogic(BaseWorkerLogic):
    ...
    async def calculate_pattern_completion(self, opportunity: Dict[str, Any]) -> float:
        ...
        - 0%: No parabolic pattern detected
        - 25%: Parabolic pattern exists but not EARLY stage
        - 50%: EARLY stage detected but weak
        - 75%: EARLY stage + good strength/acceleration
        - 100%: Perfect EARLY stage setup (strong + low exhaustion)

        Args:
            opportunity: Opportunity data with parabolic_data
        ...
        try:
            ...
            # Check if parabolic data exists (should be enriched by scanner)
            parabolic_data = opportunity.get('parabolic_data')

            if not parabolic_data:
                # No parabolic pattern detected
                return 0.0
            ...
            if parabolic_data.get('stage') == 'EARLY':
                ...
                # Not EARLY stage - pattern incomplete
                return completion
    ...
    async def should_enter(self, opportunity: Dict[str, Any]) -> bool:
        """
        Determine if parabolic entry should be executed

        Entry Logic:
        1. Anti-overtrading check
        2. Basic filters (price, quality)
        3. Parabolic pattern validation (EARLY stage)
        4. Acceleration/exhaustion checks
        5. Volume confirmation
        6. Risk manager approval

        Args:
            opportunity: Opportunity with parabolic_data

        Returns:
            True if entry approved, False otherwise
        """
        symbol = opportunity.get('symbol', 'UNKNOWN')
        ...
        try:
            ...
            # === STEP 0.5: CONTEXT ANALYSIS (Blue Sky & Fundamentals) ===
            # 1. Daily Context
            ...
            if is_blue_sky:
                 self.logger.info(f"🌤️ {symbol}: BLUE SKY PARABOLIC - Super-Extension Mode Enabled")

            # 2. Fundamental Analysis (Float & Halt)
            fundamentals = await self._analyze_smallcap_fundamentals(opportunity)
            
            # A. HALT RISK CHECK (CRITICAL for Parabolic)
            if fundamentals.get('is_halt_risk', False):
                self.logger.warning(f"🛑 {symbol}: ABORT PARABOLIC - Too close to LULD Halt Band ({fundamentals['dist_to_halt']:.1f}%)")
                return False

            # B. HIGH ROTATION BOOST
            is_high_rotation = fundamentals.get('is_high_rotation', False)
            if is_high_rotation:
                 self.logger.info(f"🚀 {symbol}: HIGH ROTATION PARABOLIC ({fundamentals['rotation_factor']:.1f}x) - Fuel Confirmed")
                 # Boost Quality Score internally
                 opportunity['quality_score'] = opportunity.get('quality_score', 0) + 15
            ...
            if bars and len(bars) >= 10:
                is_valid_vwap, vwap_reason, vwap_data = self.validate_vwap_direction(
                    ...
                    min_slope_pct=0.15,  # Higher threshold for parabolic (strong momentum required)
                    tolerance_pct=1.0,   # More flexible (breakouts can be extended)
                    symbol=symbol
                )
                ...
                self.logger.warning(f"⚠️ {symbol}: Insufficient bars for VWAP validation, skipping check")
            # ========== END VWAP VALIDATION ==========

            # === STEP 2: Get parabolic data ===
            parabolic_data = opportunity.get('parabolic_data')

            if not parabolic_data:
                self.logger.info(f"ℹ️ {symbol}: SKIPPING - No parabolic pattern detected by scanner (not enriched)")
                return False
            ...
            if stage != 'EARLY':
                self.logger.info(
                    f"⏭️ {symbol}: Stage={stage} (require EARLY for entry)"
                )
                return False
            ...
            if acceleration < self.min_acceleration:
                self.logger.debug(
                    f"⏭️ {symbol}: Acceleration {acceleration:.2f} < {self.min_acceleration:.2f}"
                )
                return False
            ...
            # Dynamic Exhaustion Limit: If Blue Sky or High Rotation, allow more exhaustion
            # (Super-parabolic moves often look exhausted early)
            current_max_exhaustion = self.max_exhaustion_for_entry
            if is_blue_sky or is_high_rotation:
                 current_max_exhaustion += 0.15 # Allow +15% more exhaustion
                 self.logger.info(f"⚡ {symbol}: Relaxing exhaustion limit {self.max_exhaustion_for_entry:.2f} -> {current_max_exhaustion:.2f} (Context)")

            if exhaustion >= current_max_exhaustion:
                self.logger.warning(
                    f"⏭️ {symbol}: Exhaustion {exhaustion:.2f} >= {current_max_exhaustion:.2f} "
                    f"(too tired for entry)"
                )
                return False
            ...
            if strength < self.min_strength:
                self.logger.debug(
                    f"⏭️ {symbol}: Strength {strength:.2f} < {self.min_strength:.2f}"
                )
                return False
            ...
            if volume_score < self.min_volume_confirmation:
                self.logger.debug(
                    f"⏭️ {symbol}: Volume score {volume_score:.2f} < {self.min_volume_confirmation:.2f}"
                )
                return False

            # === STEP 9: Risk Manager approval ===
            risk_approved = await self._check_risk_approval(symbol)
            if not risk_approved:
                self.logger.warning(f"⏭️ {symbol}: Risk manager rejected trade")
                return False
            ...
            self.logger.info(
                ...
                f"Strength={strength:.2f}, Q={quality_score:.0f}, Vol={volume_score:.2f}"
            )

            # Log reasons
            reasons = parabolic_data.get('reasons', [])
    ...
    async def _fetch_fresh_bars_for_monitoring(self, symbol: str) -> list:
        ...
        """
        try:
            # Use the execution engine's IBKR adapter to fetch fresh bars
            if hasattr(self.execution_engine, 'ibkr_adapter'):
                bars = await self.execution_engine.ibkr_adapter.get_bars(
                    symbol=symbol,
                    timeframe='1min',
                    bar_count=30  # Last 30 minutes
                )
                return bars if bars else []
            else:
                self.logger.warning(f"⚠️ {symbol}: No IBKR adapter available for fresh bars")
                return []
