...
class WorkerBasedStrategyEngine:
    ...
    async def initialize(self):
        ...
        try:
            ...
            # Worker 18: Smallcap VWAP Runner
            # Read 'enabled' from [SMALLCAP_VWAP_RUNNER] section in config.ini
            smallcap_vwap_runner_enabled = self.config_parser.getboolean('SMALLCAP_VWAP_RUNNER', 'enabled', fallback=True)
            
            if smallcap_vwap_runner_enabled:
                 self.workers['smallcap_vwap_runner'] = SmallcapVwapRunnerWorker(
                     execution_engine=self.execution_engine,
                     risk_manager=self.risk_manager,
                     config=self.config_parser
                 )
                 self.logger.info("✅ Smallcap VWAP Runner worker initialized")
    ...
    def _match_workers(self, opportunity: Dict[str, Any]) -> List[str]:
        ...
        try:
            ...
            workers = ['daily_plays', 'daily_plays_midcap', 'orb_breakout', 'vwap_breakout', 'livermore_intraday', 'vcp_smallcap', 'vcp_strict', 'balance_day', 'smallcaps_long', 'outlier_penny_extreme', 'volume_absorption', 'holy_grail', 'parabolic', 'short_parabolic', 'smallcap_vwap_runner']
            ...
            # Fallback: Still route to all workers (including new ones)
            # Fallback: Still route to all workers (including new ones)
            return ['daily_plays', 'orb_breakout', 'vwap_breakout', 'livermore_intraday', 'vcp_smallcap', 'vcp_strict', 'balance_day', 'volume_absorption', 'generic_01', 'smallcaps_long', 'outlier_penny_extreme', 'holy_grail', 'parabolic', 'short_parabolic', 'smallcap_vwap_runner']
